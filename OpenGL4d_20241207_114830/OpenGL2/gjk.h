...
struct ExpandingPolytope2
{
    ...
    {
        std::vector<glm::ivec4> expandTetras4DIndexs;
        glm::vec4 normal4D;
        float distance;
        ...
        {
            if (dot(this->ePTetras[i].normal4D, expandPoint4D - this->vertices4D[this->ePTetras[i].tetra4DIndexs[0]]) > 0.00004f)
            {
                expandTetras4DIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[2], expandVertexIndex));
                expandTetras4DIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[3], expandVertexIndex));
                expandTetras4DIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[2], ePTetras[i].tetra4DIndexs[3], expandVertexIndex));
                expandTetras4DIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[2], ePTetras[i].tetra4DIndexs[3], expandVertexIndex));
                this->ePTetras.erase(this->ePTetras.begin() + i);
                i--;
            }
        }

        for (size_t i(0); i < expandTetras4DIndexs.size(); i++)
        {
            bool isOverlap(false);
            for (size_t j(0); j < expandTetras4DIndexs.size(); j++)
            {
                if (i != j)
                {
                    if (expandTetras4DIndexs[i] == expandTetras4DIndexs[j])
                    {
                        isOverlap = true;
                        break;
                    }
            ...
            if (!isOverlap)
            {
                normal4D = normalize(cross4d(this->vertices4D[expandTetras4DIndexs[i][3]] - this->vertices4D[expandTetras4DIndexs[i][0]], this->vertices4D[expandTetras4DIndexs[i][2]] - this->vertices4D[expandTetras4DIndexs[i][0]], this->vertices4D[expandTetras4DIndexs[i][1]] - this->vertices4D[expandTetras4DIndexs[i][0]]));
                distance = dot(normal4D, this->vertices4D[expandTetras4DIndexs[i][0]]);
                this->ePTetras.push_back(EPTetra(expandTetras4DIndexs[i], glm::sign(distance) * normal4D, abs(distance)));
            }
        }
    }
};
