...
void Game::startDemos()
{
	...
	this->demos4D.push_back(new Demo4D(new Stick4D(glm::vec4(0.f), 0, VectorIndex(0, false), this->stickLength, WHITECONCRETE, this->buildColor, DEMOD4)));
	this->demos4D.push_back(new Demo4D(new Spring4D(glm::vec4(0.f), 0, VectorIndex(0, false), this->springLength, this->springStrength, STONE, this->buildColor, DEMOD4)));
	...
	camera(60.f, static_cast<float>(this->framebufferWidth) / this->framebufferHeight, 0.1f)
{
	...
	this->hitIndex = VectorIndex(0, false);
		...
		if (!(this->buildMode == BUILD_STICK || this->buildMode == BUILD_SPRING))
		{
			if (this->isFindPoint) {
				...
				if (this->buildMode == BUILD_POINTLIGHT)
				{
					...
					PointLight4D* p(this->demoObjects4D->pointLight4D);
					if (p != nullptr)p->render(this->particleTexs3D[p->texture3DIndex], this->shaders[PARTICLED4]);
					glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				}
			}
		}
