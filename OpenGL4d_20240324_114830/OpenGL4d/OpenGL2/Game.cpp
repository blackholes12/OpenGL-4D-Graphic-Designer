		...
		//if (fixedUpdate::is_fixed_update2(this->dt, 1.f / 30.f))
		{
			...
			if (this->isFindPoint) {
				...
				if (this->buildMode == BUILD_BILLIARD) {
					...
					{
						this->demoObjects4D->rigidBodies4D[i]->set_mesh_position4D(this->demos4D[27]->rigidBodies4D[i]->position4D + this->hittingPos4D + glm::vec4(0.f, 0.0525f / 2.f - 0.01f, 0.f, 0.f));
					}
				}
				if (this->buildMode == BUILD_DYNAMITE) {
					...
					if (this->isRigidBody)
					{
						...
						for (int i = 0; i < this->demoObjects4D->rigidBodies4D.size(); i++)
						{
							this->demoObjects4D->rigidBodies4D[i]->set_mesh_position4D(this->demos4D[29]->rigidBodies4D[i]->position4D + this->hittingPos4D + glm::vec4(0.f, - 0.01f, 0.f, 0.f));
						}
					}
				}
			}
