...
static CollisionConstraint collisionConstraint(
	...
	float invLA =
		massAdjustmentA * inverse_mass_term(a, manifold.normal, manifold.contacts);
	float invLB =
		massAdjustmentB * inverse_mass_term(b, manifold.normal, manifold.contacts);

	float normalMass =
		1.f / (invAMass + invBMass + invLA + invLB);

	float tangentMass[3] = { 0.0, 0.0, 0.0 };
	for (int i = 0; i < 3; i++) {
		float invLTA = massAdjustmentA
			* inverse_mass_term(a, tangents[i], manifold.contacts);
		float invLTB = massAdjustmentB
			* inverse_mass_term(b, tangents[i], manifold.contacts);

		tangentMass[i] =
			1.f / (invAMass + invBMass + invLTA + invLTB);
	}
