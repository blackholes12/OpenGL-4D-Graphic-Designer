...
public:
	...
	//		"resourcefile/images/maps/222.png"));
	//	glBindTexture(GL_TEXTURE_2D, this->textures[1]->getID());
	//	std::vector<GLubyte> textureData(200 * 4, 0);
	//	glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData.data());
	//	for (int x(0); x < 200; x++)
	//	{
	//		float r = static_cast<float>(textureData[4 * x]) / 256.f;
	//		float g = static_cast<float>(textureData[4 * x + 1]) / 256.f;
	//		float b = static_cast<float>(textureData[4 * x + 2]) / 256.f;
	//		std::cout << "glm::vec3(" << r << "," << g << "," << b << ")," << '\n';
	//	}
	//}
};
...
public:
	Texture3D(const char* fileName)
	{
		...
		if (this->pixelData)
		{
			...
			if (this->height == this->width * this->width / 36) {
				glTexImage3D(this->target, 0, GL_RGBA, this->width, this->width / 6, this->width / 6, 0, GL_RGBA, GL_UNSIGNED_BYTE, this->pixelData);
			}
			glGenerateMipmap(this->target);
		}
		else
	...
	void set_warp(unsigned char warpType)
	{
		glBindTexture(this->target, this->id);
		if (warpType == 0)
		{
