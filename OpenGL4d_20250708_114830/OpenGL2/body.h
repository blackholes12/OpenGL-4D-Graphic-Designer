...
#include"mod.h"
    ...
    static float get_moment_inertia_scalar(RigidBody4D* a, alg::bivec4 angularVelocity4D)
    {
	...
	static inline glm::vec3 color_from_temperature(float temp)
	{
		if (temp > 1000.f)
		{
			...
			idx = alg::max(0.f, idx);
			return alg::max(glm::vec3(0.f), temp * glm::mix(black_body_color[int(floor(idx))], black_body_color[int(ceil(idx))], glm::fract(idx)) / 1000.f);
		}
		else
		{
	...
	static glm::vec3 calculate_blackbody_color(float temperature)
	{
		return 2000.f*3500.f * glm::vec3(0.00708990301660023f / exp(20559.0062111801f / temperature - 1.f),
			0.0245340706670153f / exp(26352.8737370923f / temperature - 1.f),
			0.0758043913399572f / exp(33022.726819243f / temperature - 1.f));
	}