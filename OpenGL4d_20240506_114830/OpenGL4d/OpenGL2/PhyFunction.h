...
static CollisionConstraint collisionConstraint(
	...
	for (int i = 0; i < 3; i++) {
		float invLTA = massAdjustmentA* inverse_mass_term(a, tangents[i], manifold.contacts);
		float invLTB = massAdjustmentB* inverse_mass_term(b, tangents[i], manifold.contacts);
...
static void solve(CollisionConstraint* c, RigidBody4D* a, RigidBody4D* b, float dt)
{
	...
	for (int i = 0; i < 3; i++)
	{
		glm::vec4 impulse = c->tangents[i] * (newImpulses[i] - c->contacts.tangentImpulse[i]);
	...
	bool isInverse)
{
	...
	if (depth > 0.f)
	{
		glm::vec4 normal4D = normalize(normal4D0);
		glm::vec4 contacts;
		contacts = (worldPosition1 + worldPosition2)/2.f;
...
static vector<Collision*> detect_rigidbody_or_wall(Object4D* a, Object4D* b, int index1, int index2, vector<Collision*> collisions)
{
	switch (b->colType){
	    ...
	    case BOX4D:{
	    	switch (a->colType){
	    	    case SPHERE4D:{collisions = cube_to_sphere(a, b, index1, index2, collisions, true);break;}
	    ...
	    case MESH4D:{
	    	switch (a->colType){
	    	    case SPHERE4D:{collisions = mesh_to_sphere(a, b, index1, index2, collisions, true);break;}
	    	    case BOX4D:{collisions = mesh_to_cube(a, b, index1, index2, collisions, true);break;}
	    	    case STEP4D:{collisions = mesh_to_cube(a, b, index1, index2, collisions, true);break;}
	    ...
	    case CAPSULE4D:{
	    	switch (a->colType){
	    	    case SPHERE4D:{collisions = capsule_to_sphere(a, b, index1, index2, collisions, true);break;}
	    	    case BOX4D:{collisions = capsule_to_cube(a, b, index1, index2, collisions, true);break;}
	    	    case STEP4D:{collisions = capsule_to_cube(a, b, index1, index2, collisions, true);break;}
				case MESH4D:{collisions = capsule_to_mesh(a, b, index1, index2, collisions, true);break;}
	    	    case CAPSULE4D:{collisions = capsule_to_capsule(a, b, index1, index2, collisions, true);break;}
	    	}
	    	break;
	    }
	}
	return collisions;
	for (Collision* i : collisions) { free(i), i = nullptr; }
}
