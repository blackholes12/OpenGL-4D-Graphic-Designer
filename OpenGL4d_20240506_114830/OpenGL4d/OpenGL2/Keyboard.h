			...
			if (this->inputs[2]->getMouseOnPress(this->window, GLFW_MOUSE_BUTTON_2))
			{
				if (this->isFindPoint) {
					...
					if (this->buildMode == BUILD_RIGIDBODY)
					{
						...
						{
							this->pointLights4D.push_back(new PointLight4D(this->particleTexs3D[PARTICLE1], glm::vec4(), this->rigidBodies4D.size() - 1, this->lightIntensity, this->lightColor, this->lightRadius));
					...
					if (this->buildMode == BUILD_BILLIARD)
					{
						shift_animation4D(this->shifts4D, this->floats4D, this->rotates4D, this->rigidBodies4D.size(), 1);
						shift_joint4d(this->sticks4D, this->springs4D, this->hingeConstraints, this->rigidBodies4D.size(), 1);
						generate_billiard4D(this->hittingPos4D + glm::vec4(0.f, 0.0525f/2.f, 0.f, 0.f),this->initialVelocity,this->initialAngularVelocity, this->wallTexs3D[this->buildTexture], &this->rigidBodies4D, this->isGravity,this->metal,this->buildColor, this->shaders[this->buildShader]);
					}
					if (this->buildMode == BUILD_DYNAMITE)
					{
						...
						if (this->isRigidBody)
						{
							...
							generate_dynamite4D(this->hittingPos4D, this->initialVelocity,this->initialAngularVelocity,this->wallTexs3D[WHITECONCRETE], &this->dynamites4D2, this->isGravity, this->shaders[this->buildShader]);
						}
					}
					update_render_indices(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D, this->dynamites4D, &this->renderIndices1, &this->renderIndices2, this->camera.position4D);
				}
