...
void Game::startTextures()
{
	...
	this->particleTexs3D.push_back(new Texture3D("resourcefile/images/particle/PARTICLE2.png"));
	...
	this->wallTexs3D.push_back(new Texture3D("resourcefile/images/texture/OLDWOODX.png"));
	this->wallTexs3D.push_back(new Texture3D("resourcefile/images/texture/OLDWOODY.png"));
	this->wallTexs3D.push_back(new Texture3D("resourcefile/images/texture/OLDWOODZ.png"));
...
void Game::startSounds()
{
	...
	this->musics.push_back(new Music("resourcefile/sounds/musics/Stellardrone - In Time [mqms2].ogg"));
	this->musics.push_back(new Music("resourcefile/sounds/musics/Stellardrone - Rendezvous With Rama [mqms2].ogg"));
	this->musics.push_back(new Music("resourcefile/sounds/musics/Stellardrone - Light Years [mqms2].ogg"));
}
...
void Game::startDemos()
{
	...
	generate_billiard4D(glm::vec4(0.f), glm::vec4(),alg::bivec4(),this->wallTexs3D[this->buildTexture], &billiard4D, this->isGravity, 0.f,glm::vec4(1.f),this->shaders[DEMOD4]);
	...
	generate_dynamite4D(glm::vec4(0.f), this->initialVelocity, this->initialAngularVelocity, this->wallTexs3D[WHITECONCRETE], &dynamite4D2, this->isGravity, this->shaders[DEMOD4]);
...
{
	this->groups.clear();
	this->groups = pack_group(this->groups,&this->walls4D, &this->terrains4D,&this->pointLights4D);
}
...
void Game::startLevel(int level, const char* filePath)
{
	...
	this->startGroups();
	update_render_indices(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D, this->dynamites4D, &this->renderIndices1, &this->renderIndices2, this->camera.position4D);
		...
		for (Group* i : this->groups) {
			if (i->isRender)
			{
				for (Object4D* j : i->walls4D)
				{
					...
					if (dot(this->camera.forward4D, this->relativePos4D) < -lengthOfScale / 2.f)
					{
						j->isFront = false;
					}
					else {
				...
				{
					float size = 8;
					this->relativePos4D = j->position4D + j->primitive4D.vertexData4D[11904] - this->camera.position4D;
					if (dot(this->camera.forward4D, this->relativePos4D) < -size)
					{
						j->isFront = false;
					}
					else {
			...
			if (i->isRender)
			{
				for (Object4D* j : i->walls4D)
				{
					if (j->isFront) {
						float lengthOfScale = length(j->scale4D);
						this->relativePos4D = j->position4D - this->camera.position4D;
						if (abs(dot(this->camera.wDir, this->relativePos4D)) > lengthOfScale / 2.f + (float)this->objectLoadDistance)
				...
				for (Object4D* j : i->terrains4D)
				{
					if (j->isFront) {
						float lengthOfScale = length(j->scale4D);
						this->relativePos4D = j->position4D - this->camera.position4D;
						if (abs(dot(this->camera.wDir, this->relativePos4D)) > lengthOfScale + (float)this->objectLoadDistance)
...
Game::~Game()
{
	...
	glfwTerminate();
	end_imgui();
	this->shader_2D_Shape->~Shader(); delete this->shader_2D_Shape;this->shader_2D_Shape = nullptr;
	this->rectangle2D->~Rectangle2D(); delete this->rectangle2D;this->rectangle2D = nullptr;
	for (Shader* i : this->shaders) { i->~Shader(); delete i; i = nullptr; }
	for (Texture* i : this->textures) { i->~Texture(); delete i; i = nullptr; }
	for (Texture* i : this->pointerTexs) { i->~Texture(); delete i; i = nullptr; }
	this->deleteMap3D->~DeleteMap3D(); delete this->deleteMap3D; this->deleteMap3D = nullptr;
	for (Texture3D* i : this->textures3D) { i->~Texture3D(); delete i; i = nullptr; }
	for (Texture3D* i : this->particleTexs3D) { i->~Texture3D(); delete i;  i = nullptr; }
	for (Texture3D* i : this->wallTexs3D) { i->~Texture3D(); delete i; i = nullptr; }
	for (Texture3D* i : this->groundTexs3D) { i->~Texture3D(); delete i; i = nullptr; }
	for (Sound* i : this->sounds) { i->~Sound(); delete i; i = nullptr; }
	for (Music* i : this->musics) { i->~Music(); delete i; i = nullptr; }
	for (Object* i : this->objects) { i->~Object(); delete i; i = nullptr; }
	this->skybox4D->~Sky_Box4D(); delete this->skybox4D; this->skybox4D = nullptr;
	this->coordinate4D->~Coordinate4D(); delete this->coordinate4D; this->coordinate4D = nullptr;
	this->targetIcon->~Rectangle2D(); delete this->targetIcon; this->targetIcon = nullptr;
	this->demoObjects4D->~Demo4D(); delete this->demoObjects4D; this->demoObjects4D = nullptr;
	for (Demo4D* i : this->demos4D) { i->~Demo4D(); delete i; i = nullptr; }
	for (JointLine4D* i : this->dragLines4D) { i->~JointLine4D(); delete i; i = nullptr; }
	for (Input* i : this->inputs) { i->~Input(); delete i; i = nullptr; }
	for (Wall4D* i : this->walls4D) { i->~Wall4D(); delete i; i = nullptr; }
	for (Wall4D* i : this->dynamites4D) { i->~Wall4D(); delete i; i = nullptr; }
	for (RigidBody4D* i : this->dynamites4D2) { i->~RigidBody4D(); delete i; i = nullptr; }
	for (RigidBody4D* i : this->rigidBodies4D) { i->~RigidBody4D(); delete i; i = nullptr; }
	for (Terrain4D* i : this->terrains4D) { i->~Terrain4D(); delete i; i = nullptr; }
	for (Water4D* i : this->waters4D) { i->~Water4D(); delete i; i = nullptr; }
	for (Group* i : this->groups) { i->~Group(); delete i; i = nullptr; }
	for (Emitter4D* i : this->emitters4D) { i->~Emitter4D(); delete i; i = nullptr; }
	for (RenderIndex* i : this->renderIndices1) { delete i; i = nullptr; }
	for (RenderIndex* i : this->renderIndices2) { delete i; i = nullptr; }
	for (PointLight* i : this->pointLights) { i->~PointLight(); delete i; i = nullptr; }
	for (DirectionLight* i : this->directionLights) { i->~DirectionLight(); delete i; i = nullptr; }
	for (PointLight4D* i : this->pointLights4D) { i->~PointLight4D(); delete i; i = nullptr; }
	this->directionLight4D->~DirectionLight4D(); delete this->directionLight4D; this->directionLight4D = nullptr;
	for (Collision* i : this->collisions) { delete i; i = nullptr; }
	for (HingeConstraint4D* i : this->hingeConstraints) { delete i; i = nullptr; }
	for (Stick4D* i : this->sticks4D) { i->~Stick4D(); delete i; i = nullptr; }
	for (Spring4D* i : this->springs4D) { i->~Spring4D(); delete i; i = nullptr; }
	for (Shift4D* i : this->shifts4D) { i->~Shift4D(); delete i; i = nullptr; }
	for (Float4D* i : this->floats4D) { i->~Float4D(); delete i; i = nullptr; }
	for (Rotate4D* i : this->rotates4D) { i->~Rotate4D(); delete i; i = nullptr; }
}
...
void Game::render0()
{
	...
	for (int index=0;index<this->renderIndices1.size();index++)
	{
	...
	//{
	//	if(g->isRender)
	//	for (PointLight4D* p : g->pointLights4D) {
	//		glDepthMask(false);
	//		p->render(this->shaders[PARTICLE4D]);
	//		if (this->frameType == 1)
	//		{
	//			glDepthMask(true);
	...
	if (ray::is_in_screen(-this->directionLight4D->direction4D, camera.viewMat4D, camera.projectionMat))
	{
		...
		{
			glDepthMask(false);
			this->directionLight4D->render_lens_flare(this->camera.forward4D,this->particleTexs3D[PARTICLE2],this->shaders[PARTICLE4D]);
			glDepthMask(true);
		}
	}
	//
