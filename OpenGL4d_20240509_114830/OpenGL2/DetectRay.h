				...
				for (size_t z(0); z < size; z++)
				{
					for (size_t w(0); w < size; w++)
					{
						...
						if (dot(glm::vec3(right.x, right.z, right.w), glm::vec3(relativePos2.x, relativePos2.z, relativePos2.w)) < 0.5f * sqrt(3))
						{
							...
							float t(dot(forwardXZW, glm::vec3(relativePos2.x, relativePos2.z, relativePos2.w)) / dot(forwardXZW, forwardXZW));
							float t1(t - 0.5f * sqrt(3));
							float t2(t + 0.5f * sqrt(3));
	...
	*/
	...
	glm::vec4* hitPosNear)
	{
		...
		for (size_t i(1); i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++) {
			Object4D* a(find_rigidbody4d(rigidBodies4D, dynamites4D2, i));
			...
			{
				if (ray::ray_detect_collider4D(a, camPos, camFor, camRight, hitPosNear, &hitPosFar))
				{
		...
		if (isDetectWall)
		{
			for (size_t i(0); i < size_of_walls4d(groups); i++) {
				...
				{
					Wall4D* a(find_wall4d(groups, i));
					if (!(a->diffuse3D->isTransparent||a->paintingColor.w<1.f) || isDetectTransparent)
					{
						float colRadious(length(a->scale4D) / 2.f);
						float distance4D(length(a->position4D - camPos));
						float cosView(dot(camFor, normalize(a->position4D - camPos)));
			...
			for (size_t i(0); i < size_of_objects4d(walls4D,dynamites4D); i++) {
				Object4D* a(find_wall4d(walls4D, dynamites4D, i));
				float colRadious(length(a->scale4D) / 2.f);
				float distance4D(length(a->position4D - camPos));
				float cosView(dot(camFor, normalize(a->position4D - camPos)));
				float sinView(sqrt(1.f - cosView * cosView));
