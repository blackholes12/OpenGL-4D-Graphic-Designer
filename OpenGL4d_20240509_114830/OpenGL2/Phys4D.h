...
namespace phy {
	...
	static void update_rigidbody_velocity4D(RigidBody4D* a, const float dt)
	{
		if (!a->is_static())
		{
			alg::bivec4 delta_angular_vel(alg::bivec4(0.f,0.f,0.f,0.f,0.f,0.f));
			for (ForceSystem4D* j : a->forceSystems4D)
			{
				a->velocity4D += inverse_moment_of_mass(a, j->force4D, dt);
				if (j->position4D != glm::vec4(0.f))
				{
					delta_angular_vel = delta_angular_vel + inverse_moment_of_inertia(a,wedge_v(alg::vec4(j->position4D), rotate(a->rotation4D.reverse(), alg::vec4(dt * j->force4D))));
				}
			}
	...
	static void caculate_rigidBodies4D(std::vector<RigidBody4D*> rigidBodies4D, std::vector<RigidBody4D*> dynamites4D2, std::vector<Group*> groups, vector<Wall4D*> walls4D,vector<Wall4D*> dynamites4D, std::vector<Water4D*> waters4D, std::vector<Collision>* collisions, const float dt)
	{
		...
		for (Collision collision : *collisions)
		{
			...
			else if (collision.index2 != -1)
			{
				...
				else if (collision.index2 >= size_of_objects4d(rigidBodies4D, dynamites4D2))
				{
		...
		for (Constraint* i : constraints) { free(i), i = nullptr; }
		glm::vec4 dir_a_to_b;
		float dis;
		...
		const float dt)
	{
		...
		update_joints4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, sticks4D, springs4D, hingeConstraints, dt);
