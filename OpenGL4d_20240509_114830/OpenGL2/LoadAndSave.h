	...
	vector < PointLight4D*> pointLights4D,const float worldTime)
{
	...
	for (size_t i(0); i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++)
	{
		...
		std::cout <<type<< " " << name_index(r->objectName) <<" " << primitive_index(r->primitive4D.primitiveName) <<" " <<
			...
			r->colType<<" "<< shader_list(r->shader, shaders)<<" "<<
	...
	for (size_t i(0); i < hingeConstraints.size(); i++)
	{
		...
		while (std::getline(in_file, line2))
		{
			...
			if (prefix2 == 0|| prefix2 == 1) //rigid body
			{
				...
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					...
					colType >> shader>> animatePosition4D.x>> animatePosition4D.y>> animatePosition4D.z>> animatePosition4D.w;
				if(prefix2 == 0)
				    rigidBodies4D->push_back(new RigidBody4D(name_list(nameIndex), primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mass, momentInertiaScalar, mu, restitution, isGravity, isFrozen, velocity4D, angularVelocity4D, isSpecular, wallTextures3D[diffuse3D], metal, paintingColor, colType, shaders[shader], animatePosition4D,true));
			    if(prefix2 == 1)
			...
			{
				unsigned nameIndex;unsigned primitiveIndex;glm::vec4 primitiveScale4D;glm::vec4 position4D;alg::rotor4 rotation4D;glm::vec4 scale4D;float mu;float restitution;bool isSpecular;unsigned diffuse3D;float metal;glm::vec4 paintingColor;unsigned int colType;unsigned shader;
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					position4D.x >> position4D.y >> position4D.z >> position4D.w >>
					rotation4D.s >> rotation4D.b.xy >> rotation4D.b.xz >> rotation4D.b.xw >> rotation4D.b.yz >> rotation4D.b.yw >> rotation4D.b.zw >> rotation4D.q.xyzw >>
			...
			{
				unsigned nameIndex;glm::vec4 position4D;glm::vec4 scale4D;float mu;float restitution;unsigned int colType;unsigned shader;
				ss2 >> nameIndex >> 
					position4D.x >> position4D.y >> position4D.z >> position4D.w >>
			...
			if (prefix2 == 5) //water
			{
				unsigned nameIndex;unsigned primitiveIndex;glm::vec4 primitiveScale4D;glm::vec4 position4D;glm::vec4 scale4D;unsigned int colType;unsigned shader;
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					position4D.x >> position4D.y >> position4D.z >> position4D.w >>
			...
			if (prefix2 == 6)
			{
				glm::vec4 fixPoint4D;int index1;int index2;float stickLength;unsigned diffuse3D;glm::vec4 paintingColor;unsigned shader;
			...
			if (prefix2 == 7)
			{
				glm::vec4 fixPoint4D;int index1;int index2;float springLength;float k;unsigned diffuse3D;glm::vec4 paintingColor;unsigned shader;
			...
			if (prefix2 == 11)
			{
				glm::vec4 worldPosition4D;glm::vec4 bodyPosition4D1;glm::vec4 bodyPosition4D2;int index1;int index2;
			...
			if (prefix2 == 12)
			{
				unsigned diffuse3D;glm::vec4 position4D;int index;float intensity;glm::vec3 color;float radius;
				ss2 >> diffuse3D >> position4D.x >> position4D.y >> position4D.z >> position4D.w >> index >> intensity >> color.x >> color.y >> color.z >>
					radius;
				pointLights4D->push_back(new PointLight4D(particleTextures3D[diffuse3D], position4D, index, intensity, color, radius));
			}
			if (prefix2 == 13)
			{
				ss2 >> *worldTime;
