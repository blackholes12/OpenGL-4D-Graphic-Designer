...
static CollisionConstraint collisionConstraint(
	...
	float normalMass =
		0.5f / (invAMass + invBMass + invLA + invLB);
	...
	for (int i = 0; i < 3; i++) {
		...
		tangentMass[i] =
			0.5f / (invAMass + invBMass + invLTA + invLTB);
	}
...
static void solve(CollisionConstraint c, RigidBody4D* a, RigidBody4D* b, float dt)
{
	...
	// calculate normal impulse
	prevImpulse = c.contacts.normalImpulse;
	c.contacts.normalImpulse = max(prevImpulse + lambda0, 0.f);
	...
	float dt)
{
	if (collision->collisionManifold.depth > 0.f)
	{
		...
		{
			solve(collisionConstraint(collision->collisionManifold, a, collision->massAdjustmentA, a->mu, b, collision->massAdjustmentB, b->mu), a, b, dt);
		}
	}
}
	...
	float dt)
{
	if (collision->collisionManifold.depth > 0.f)
	{
		...
		{
			solve(collisionConstraint(collision->collisionManifold, a, collision->massAdjustmentA, b->mu,b->restitution), a, dt);
		}
	}
}
	...
	float dt)
{
	if (collision->collisionManifold.depth > 0.f)
	{
		solve(collisionConstraint(collision->collisionManifold, a, collision->massAdjustmentA, 1.6f,0.25f), a, dt);
	}
}
	...
	float dt)
{
	...
	if (depth > 0.f)
	{
		...
		solve(collisionConstraint(CollisionManifold(normal4D, depth, contacts), a, 1.f, INFINITY, b, 1.f, INFINITY), a, b, dt);
	}
}
	...
	float dt, bool isInverse)
{
	...
	if (depth > 0.f)
	{
		...
		solve(collisionConstraint(CollisionManifold(normal4D, depth, contacts), a, 1.f, INFINITY,0.f), a, dt);
	}
}
