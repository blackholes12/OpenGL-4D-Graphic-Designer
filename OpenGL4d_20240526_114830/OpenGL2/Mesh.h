...
private:
	...
	this->ObjectMat =
		...
		this->ObjectMat = this->ObjectMat*
			glm::mat4(cos(this->Rotate_Angle_rad.z), -sin(this->Rotate_Angle_rad.z), 0, 0,
				sin(this->Rotate_Angle_rad.z), cos(this->Rotate_Angle_rad.z), 0, 0,
				0, 0, 1.f, 0,
				0, 0, 0, 1.f)*
			glm::mat4(1.f, 0, 0, 0,
				0, cos(this->Rotate_Angle_rad.x), -sin(this->Rotate_Angle_rad.x), 0,
				0, sin(this->Rotate_Angle_rad.x), cos(this->Rotate_Angle_rad.x), 0,
				0, 0, 0, 1.f)*
			glm::mat4(cos(this->Rotate_Angle_rad.y), 0, sin(this->Rotate_Angle_rad.y), 0,
				0, 1.f, 0, 0,
				-sin(this->Rotate_Angle_rad.y), 0, cos(this->Rotate_Angle_rad.y), 0,
				0, 0, 0, 1.f)
			;
		this->ObjectMat = this->ObjectMat
			* glm::mat4(this->scale.x, 0, 0, 0,
				0, this->scale.y, 0, 0,
				0, 0, this->scale.z, 0,
				0, 0, 0, 1.f)
			;
...
private:
	...
	void initVAO()
	{
		...
		//CubeIndex xzw For The Terrain 4D
		glVertexAttribPointer(12, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex4D), (GLvoid*)offsetof(Vertex4D, cubeIndex_XZW));
		glEnableVertexAttribArray(12);
		//ID
		glVertexAttribPointer(13, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex4D), (GLvoid*)offsetof(Vertex4D, id));
	...
	void update_mesh4D_mat()
	{
		this->mesh4DMat = to_matrix(this->rotation4D) *
		    ...
		    glm::mat4(this->scale4D.x, 0, 0, 0,
				0, this->scale4D.y, 0, 0,
				0, 0, this->scale4D.z, 0,
				0, 0, 0, this->scale4D.w)
			;
	}
...
///////////////////////
	...
	~Mesh4D()
	{
		...
		delete[] this->vertexArray4D,this->vertexArray4D = nullptr;
		delete[] this->indexArray4D,this->indexArray4D = nullptr;
		delete[] this->vertexFrameArray4D,this->vertexFrameArray4D = nullptr;
		delete[] this->indexFrameArray4D,this->indexFrameArray4D = nullptr;
	}
	...
	void render(Shader* shader)
	{
		...
		//RENDER
		if (this->sizeOfIndices4D == 0)
			glDrawArrays(GL_TRIANGLES, 0, (GLsizei)this->sizeOfVertices4D);
		if (this->sizeOfIndices4D != 0)
			glDrawElements(GL_TRIANGLES, (GLsizei)this->sizeOfIndices4D, GL_UNSIGNED_INT, 0);
	}
	void render2(Shader* shader2)
	{
		...
		glBindVertexArray(this->VAO2);
		if (this->sizeOfIndices4D == 0)
		glDrawArrays(GL_LINES, 0,(GLsizei)this->sizeOfFrameIndices4D);
		if (this->sizeOfIndices4D != 0)
