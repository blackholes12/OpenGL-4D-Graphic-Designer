	...
	vector < PointLight4D*> pointLights4D,const float worldTime)
{
	...
	for (size_t i(0); i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++)
	{
		...
		std::cout <<type<< " " << name_index(r->objectName) <<" " << primitive_index(r->primitive4D.primitiveName) <<" " <<
			...
			r->isSpecular<<" "<< r->texture3DIndex <<" " << r->metal << " " <<
	...
	for (size_t i(0); i < size_of_objects4d(groups, walls4D, dynamites4D); i++)
	{
		...
		std::cout << type << " " << name_index(w->objectName) << " " << primitive_index(w->primitive4D.primitiveName) << " " <<
			...
			w->mu << " " << w->restitution <<" "<<
			w->isSpecular << " " << w->texture3DIndex << " " << w->metal << " " <<
		...
		while (std::getline(in_file, line2))
		{
			...
			case 0: { 
				...
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					...
					isSpecular >> texture3DIndex >> metal >>
			...
			case 1: { 
				...
				break; }
			case 2: { 
				unsigned nameIndex; unsigned primitiveIndex; glm::vec4 primitiveScale4D; glm::vec4 position4D; alg::rotor4 rotation4D; glm::vec4 scale4D; float mu; float restitution; bool isSpecular; unsigned texture3DIndex; float metal; glm::vec4 paintingColor; unsigned int colType; unsigned shaderIndex;
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					position4D.x >> position4D.y >> position4D.z >> position4D.w >>
					rotation4D.s >> rotation4D.b.xy >> rotation4D.b.xz >> rotation4D.b.xw >> rotation4D.b.yz >> rotation4D.b.yw >> rotation4D.b.zw >> rotation4D.q.xyzw >>
					...
					paintingColor.x >> paintingColor.y >> paintingColor.z >> paintingColor.w >>
					colType >> shaderIndex;
				walls4D->push_back(new Wall4D(name_list(nameIndex), primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mu, restitution, isSpecular, texture3DIndex, metal, paintingColor, colType, shaderIndex));
				break; }
			case 3: { 
				unsigned nameIndex; unsigned primitiveIndex; glm::vec4 primitiveScale4D; glm::vec4 position4D; alg::rotor4 rotation4D; glm::vec4 scale4D; float mu; float restitution; bool isSpecular; unsigned texture3DIndex; float metal; glm::vec4 paintingColor; unsigned int colType; unsigned shaderIndex;
				ss2 >> nameIndex >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					...
					paintingColor.x >> paintingColor.y >> paintingColor.z >> paintingColor.w >>
					colType >> shaderIndex;
				dynamites4D->push_back(new Wall4D(name_list(nameIndex), primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mu, restitution, isSpecular, texture3DIndex, metal, paintingColor, colType, shaderIndex));
				break; }
			case 4: { 
