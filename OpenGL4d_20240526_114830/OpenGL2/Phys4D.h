...
namespace phy {
	...
	static void start_rigidBodies4D(std::vector<RigidBody4D*> rigidBodies4D, std::vector<RigidBody4D*> dynamites4D2, const float dt)
	{
		for (int i(0); i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++)
		{
			...
			if (!a->is_static()&&!a->isSleep)
			{
				if (a->isGravity)
				{
					a->forceSystems4D.push_back(new ForceSystem4D(glm::vec4(0, -9.8f* a->mass, 0, 0), glm::vec4(0.f)));
				}
	...
	static void caculate_rigidBodies4D(std::vector<RigidBody4D*> rigidBodies4D, std::vector<RigidBody4D*> dynamites4D2, std::vector<Group*> groups, vector<Wall4D*> walls4D,vector<Wall4D*> dynamites4D, std::vector<Water4D*> waters4D, std::vector<std::vector<Collision>>* collisionsVec, const float dt)
	{
		...
		//	for (Collision collision : *collisionsVec)
		//	{
		...
		for (std::vector<Collision> collisions : *collisionsVec)
		{
			...
			else if (collisions[0].index2 != -1)
			{
				...
				{
					get_constraint(collisions[0].index1, collisions[0].index2, a, find_wall4d(groups, walls4D, dynamites4D, collisions[0].index2 - size_of_objects4d(rigidBodies4D, dynamites4D2)), collisions, &constraintsVec);
				}
			}
		}
		for (unsigned i(0); i < 20; i++)
		{
			for (std::vector<Constraint*> constraints : constraintsVec)
			{
				...
				{
					solve_rigidbody_edge(a, &constraints);
				}
				else if (constraints[0]->index2 != -1)
				{
					if (constraints[0]->index2 < size_of_objects4d(rigidBodies4D, dynamites4D2))
					{
						...
						if (!b->is_static())
						{
							solve_rigidbody_rigidbody(a, b, &constraints);
							wake_up_rigidBody4D(a);
							wake_up_rigidBody4D(b);
						}
						else
						{
							solve_rigidbody_wall(a, b, &constraints);
							wake_up_rigidBody4D(a);
							wake_up_rigidBody4D(b);
						}
					}
					if (constraints[0]->index2 >= size_of_objects4d(rigidBodies4D, dynamites4D2))
					{
						solve_rigidbody_wall(a, find_wall4d(groups, walls4D, dynamites4D, constraints[0]->index2 - size_of_objects4d(rigidBodies4D, dynamites4D2)), &constraints);
					}
				}
			}
		}
		...
		const float dt)
	{
		...
		for (std::vector < HingeConstraint4D> hs : hingeConstraintsVec)
		{
			if (hs[0].index2 == -1)
			{
				...
				{
					get_constraint(hs[0].index1, find_rigidbody4d(rigidBodies4D, dynamites4D2, hs[0].index1), hs,&constraintsVec);
				}
			}
			else if (hs[0].index2 != -1)
			{
				if (hs[0].index2 < size_of_objects4d(rigidBodies4D, dynamites4D2))
				{
					if (hs[0].index1 < size_of_objects4d(rigidBodies4D, dynamites4D2))
					{
						...
						if (!a->is_static() && !find_rigidbody4d(rigidBodies4D, dynamites4D2, hs[0].index2)->is_static())
						{
							get_constraint(hs[0].index1, hs[0].index2, a, b, hs, &constraintsVec);
						}
						else if (a->is_static() && b->is_static())
						{
							get_constraint(hs[0].index1, hs[0].index2, a, b, hs, &constraintsVec);
						...
						{
							get_constraint(hs[0].index1, hs[0].index2, a, b, hs, &constraintsVec,false);
						}
						else if (a->is_static() && !b->is_static())
						{
							get_constraint(hs[0].index2, hs[0].index1, b, a, hs, &constraintsVec, true);
						}
					}
				}
			}
		...
		for (unsigned i(0); i < 2; i++)
		{
			for (std::vector<Constraint*> constraints : constraintsVec)
			{
				if (constraints[0]->index2 == -1)
				{
					...
					{
						RigidBody4D* a(find_rigidbody4d(rigidBodies4D, dynamites4D2, constraints[0]->index1));
						solve_rigidbody2(a, &constraints);
					}
				}
				else if (constraints[0]->index2 != -1)
				{
					if (constraints[0]->index2 < size_of_objects4d(rigidBodies4D, dynamites4D2))
					{
						if (constraints[0]->index1 < size_of_objects4d(rigidBodies4D, dynamites4D2))
						{
							...
							if (!a->is_static() && !b->is_static())
							{
								solve_rigidbody_rigidbody2(a, b, &constraints);
							}
							else if (a->is_static() && b->is_static())
							{
								solve_rigidbody_rigidbody2(a, b, &constraints);
							...
							{
								solve_rigidbody_wall2(a, b, &constraints);
							}
							else if (a->is_static() && !b->is_static())
							{
								solve_rigidbody_wall2(b, a, &constraints);
							}
						}
					}
				}
		...
		const float dt)
	{
		...
		caculate_rigidBodies4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, waters4D, collisions, dt);
		caculate_joints4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, sticks4D, springs4D, hingeConstraintsVec, dt);
