...
static float length(alg::bivec4 angularVelocity4D)
{
	return sqrt(angularVelocity4D.xy * angularVelocity4D.xy + angularVelocity4D.xz * angularVelocity4D.xz + angularVelocity4D.xw * angularVelocity4D.xw + angularVelocity4D.yz * angularVelocity4D.yz + angularVelocity4D.yw * angularVelocity4D.yw + angularVelocity4D.zw * angularVelocity4D.zw);
}
...
static alg::rotor4 look_at(glm::vec4 from,glm::vec4 to)
{
	glm::vec4 direction4D(to - from);
	if (direction4D.x == 0.f&& direction4D.y == 0.f&& direction4D.w == 0.f)
	{
		return alg::rotor4();
	}
	else
	{
		glm::vec4 minusZ(glm::vec4(0.f, 0.f, -1.f, 0.f));
		direction4D = normalize(direction4D);
		float cosAngle(dot(minusZ, direction4D));
		float angle(acos(cosAngle));
		glm::vec4 n(normalize(direction4D - cosAngle * minusZ));
		alg::bivec4 delta(wedge_v(alg::vec4(minusZ), alg::vec4(angle * n)));
