...
static glm::vec4 get_initial_direction4D(glm::vec4 dirction4D,float scaleRate)
{
    if (abs(dirction4D.x * dirction4D.y * dirction4D.z * dirction4D.w) < 0.00001f)
    {
        if (scaleRate < 100.f && scaleRate>0.01f)
        {
            return dirction4D + glm::vec4(0.0001f, 0.0002f, 0.0003f, 0.0004f);
        }
        else
        {
            return glm::vec4(0.1f, 0.2f, 0.3f, 0.4f);
        }
    }
    else
    {
...
{
    glm::vec4 a = support_of_cso(s1, s2, get_initial_direction4D(s1->position4D-s2->position4D, length(s1->scale4D) / length(s2->scale4D)));
    *simplex = Simplex(a);
    glm::vec4 d = -a;
...
static glm::vec4 epa2(Object4D* s1, Object4D* s2, Simplex simplex, float* depth)
{
    unsigned int count = 0;
