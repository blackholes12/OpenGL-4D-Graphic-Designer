			...
			if (d->isExploding)
			{
				...
				for (size_t j(0); j < this->rigidBodies4D.size(); j++)
				{
					if (length(this->rigidBodies4D[j]->position4D - o->position4D) < 10.f)
					{
						...
						{
							erase_animation4D(&this->shifts4D, &this->floats4D, &this->rotates4D, &this->openDoors4D, j);
							shift_animation4D(this->shifts4D, this->floats4D, this->rotates4D, this->openDoors4D, j, -1);
							...
							erase_dynamite4d_indices(&this->dynamiteIndices, j);
							shift_dynamite4d_indices(this->dynamiteIndices, j, -1);
				...
				for (size_t j(0); j < this->walls4D.size(); j++)
				{
					if (this->walls4D[j]->objectName == "Can-Be-Destroyed" || this->walls4D[j]->objectName == "Glass4D")
					{
						...
						{
							erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, j + this->rigidBodies4D.size());
							shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, j + this->rigidBodies4D.size(), -1);
							erase_dynamite4d_indices(&this->dynamiteIndices, j + this->rigidBodies4D.size());
							shift_dynamite4d_indices(this->dynamiteIndices, j + this->rigidBodies4D.size(), -1);
							this->walls4D.erase(this->walls4D.begin() + j);
							this->groupIndices = shift_group(this->groupIndices, &this->groups, j, -1);
							j--;
							update_render_indices(this->rigidBodies4D, this->groupIndices, this->groups, this->walls4D, this->sticks4D, this->springs4D, &this->renderIndices1, &this->renderIndices2, this->camera.position4D);
							sort_render_indices(&this->renderIndices1); sort_render_indices(&this->renderIndices2);
							update_collide_indices(this->rigidBodies4D, this->groupIndices, this->groups, this->walls4D);
				...
				if(d->index < this->rigidBodies4D.size())
				{
					erase_animation4D(&this->shifts4D, &this->floats4D, &this->rotates4D, &this->openDoors4D, d->index);
					shift_animation4D(this->shifts4D, this->floats4D, this->rotates4D, this->openDoors4D,d->index,-1);
					erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, d->index);
					shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, d->index, -1);
					size_t pointLightSize(this->pointLights4D.size());
					erase_point_light4d(&this->pointLights4D, this->shaders[WALLD4], this->shaders[TERRAIND4], this->shaders[LIGHT_MAPD3], d->index);
					if (pointLightSize != this->pointLights4D.size())
					{
						shift_point_light4d(this->pointLights4D, d->index, -1);
						this->shaders[LIGHT_MAPD3]->use();
						this->lightMap3D->bind_image(0);
						glDispatchCompute(4, 128, 4);
						glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
				...
				else
			    {
					...
					erase_joint4d(&this->sticks4D, &this->springs4D, &this->hingeConstraintsVec, d->index);
					shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, d->index, -1);
					this->groupIndices = shift_group(this->groupIndices, &this->groups, d->index - this->rigidBodies4D.size(), -1);
			    }
				erase_dynamite4d(&this->rigidBodies4D, &this->walls4D, d);
				i--;
				erase_dynamite4d_indices(&this->dynamiteIndices, d->index);
				shift_dynamite4d_indices(this->dynamiteIndices, d->index, -1);
