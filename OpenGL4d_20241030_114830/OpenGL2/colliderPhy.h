...
static struct Collision
{
    ...
    int type;
    explicit Collision(int index1, int index2, float massAdjustmentA, float massAdjustmentB, glm::vec4 normal, float depth, glm::vec4 contactPosA,glm::vec4 contactPosB,int type = 0)
    {
        this->index1 = index1;
        this->index2 = index2;
...
static std::pair < int, int > inverse(int index1, int index2, bool isInverse)
{
    if (isInverse) return { index2, index1 };
    ...
    for (HingeConstraint4D* h : hs)
    {
        if (h->index2 == nullptr)
        {
            ...
            if (depth > 0.f)
            {
                ...
                collisions.push_back(Collision(h->index1, h->index2->n, 1.f, 1.f, normal4D, depth, contactPosA, contactPosB, 1));
            }
        }
        else
        {
            ...
            {
                glm::vec4 normal4D(alg::normalize(normal4D0));
                std::tie(h->index1, h->index2->n) = inverse(h->index1, h->index2->n, isInverse);
                std::tie(contactPosA, contactPosB) = inverse(contactPosA, contactPosB, isInverse);
                collisions.push_back(Collision(h->index1, h->index2->n, 1.f, 1.f, inverse(normal4D, !isInverse), depth, contactPosA, contactPosB, 1));
            }
        }
    }
...
static std::vector<std::vector<Collision>> sphere_to_sphere(int index1, int index2, Object4D* a, Object4D* b, std::vector<std::vector<Collision>> collisionsVec, bool isInverse)
{
        ...
        alg::abs(a->position4D.w - b->position4D.w) < (a->scale4D.w + b->scale4D.w) / 2.f)
    {
        ...
        {
            std::vector<Collision> collisions;
            glm::vec4 normal(alg::normalize(a->position4D - b->position4D));
            float depth(radiusA + radiusB - alg::length(a->position4D - b->position4D));
                ...
                for (int i(0); i < 4; i++)
                {
                        ...
                        alg::abs(pB[dir1[2]]) <= sA[dir1[2]])
                    {
                        for (int j(0); j < 2; j++)
                        {
                            ...
                            {
                                normal = glm::vec4(0.f);
                                normal[dir2] = sign * sA[dir2] - pB[dir2];
                ...
                for (int i(0); i < 6; i++)
                {
                        ...
                        alg::abs(pB[dir1[1]]) < sA[dir1[1]])
                    {
                        for (int j(0); j < 4; j++)
                        {
                            ...
                            {
                                normal = glm::vec4(0.f);
                                normal[dir2[0]] = sign[0] * sA[dir2[0]] - pB[dir2[0]];
                                normal[dir2[1]] = sign[1] * sA[dir2[1]] - pB[dir2[1]];
                                normal = alg::normalize(normal);
                                glm::vec4 aVec4(pB);
                    ...
                    if (alg::abs(pB[dir1]) < sA[dir1])
                    {
                        for (int j(0); j < 8; j++)
                        {
                                    ...
                                    pB[dir2[2]] - sign[2] * sA[dir2[2]])) < radius)
                            {
                                ...
                                normal[dir2[0]] = sign[0] * sA[dir2[0]] - pB[dir2[0]];
                                normal[dir2[1]] = sign[1] * sA[dir2[1]] - pB[dir2[1]];
                                normal[dir2[2]] = sign[2] * sA[dir2[2]] - pB[dir2[2]];
                                normal = alg::normalize(normal);
                                glm::vec4 aVec4(pB);
                                aVec4[dir2[0]] = sign[0] * sA[dir2[0]];
                                aVec4[dir2[1]] = sign[1] * sA[dir2[1]];
                    ...
                    dir2[0] = 0, dir2[1] = 1, dir2[2] = 2, dir2[3] = 3;
                    {
                        for (int j = 0; j < 16; j++)
                        {
                                    ...
                                    pB[dir2[3]] - sign[3] * sA[dir2[3]])) < radius)
                            {
                                ...
                                normal[dir2[1]] = sign[1] * sA[dir2[1]] - pB[dir2[1]];
                                normal[dir2[2]] = sign[2] * sA[dir2[2]] - pB[dir2[2]];
                                normal[dir2[3]] = sign[3] * sA[dir2[3]] - pB[dir2[3]];
                                normal = alg::normalize(normal);
                                glm::vec4 aVec4(pB);
                                aVec4[dir2[0]] = sign[0] * sA[dir2[0]];
                                aVec4[dir2[1]] = sign[1] * sA[dir2[1]];
...
static std::vector<std::vector<Collision>> capsule_to_sphere(int index1, int index2, Object4D* a, Object4D* b, std::vector<std::vector<Collision>> collisionsVec, bool isInverse)
{
        ...
        alg::abs(a->position4D.w - b->position4D.w) < capsuleRadius + sphereRadius)
    {
        ...
        {
            glm::vec4 normal(alg::normalize(pA1 - pBToA));
        ...
        {
            glm::vec4 normal(alg::normalize(pA2 - pBToA));
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            for (int j(0); j < cubes4DB.size(); j++)
                            {
                                ...
                                {
                                    contactPosA = pA1 - radiusA * normalToA;
                                    contactPosB = world_pos_to_body(b, body_pos_to_world(a, pA1 + radiusA * normalToA + depth * normal));
                                    std::tie(contactPosA, contactPosB) = inverse(contactPosA, contactPosB, isInverse);
                                    collisions.push_back(Collision(index1, index2, 0.f, 0.f, inverse(normal, isInverse), depth, contactPosA, contactPosB));
                                    contactPosA = pA2 - radiusA * normalToA;
                                    contactPosB = world_pos_to_body(b, body_pos_to_world(a, pA2 + radiusA * normalToA + depth * normal));
                                    std::tie(contactPosA, contactPosB) = inverse(contactPosA, contactPosB, isInverse);
                                    collisions.push_back(Collision(index1, index2, 0.f, 0.f, inverse(normal, isInverse), depth, contactPosA, contactPosB));
                                }
                            }
