...
static CollisionConstraint collisionConstraint(
	...
	for (int i(0); i < 4; i++) {
		float invLA(massAdjustmentA * inverse_mass_term2(a, orthonormalBasis[i], manifold.contactPosB));
		float invLB(massAdjustmentB * inverse_mass_term2(b, orthonormalBasis[i], manifold.contactPosA));
		orthonormalBasisInv[i] = glm::vec3(invAMass + invBMass, invLA, invLB);
	}
	return CollisionConstraint(orthonormalBasis, ContactState(manifold.contactPosA, manifold.contactPosB, bias, orthonormalBasisInv, glm::vec4(0.f)), friction);
}
...
static void solve(std::vector <Constraint*>* cs, RigidBody4D* a, RigidBody4D* b, bool isInverseResolve)
{
	for (unsigned int i(0);i<cs->size();i++)
	{
		...
		orthonormalBasisLambda[0] = dot(-relVel, cC->orthonormalBasis[0]) / (cC->contacts.orthonormalBasisInv[0].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[0].y/a->momentInertiaScalar:0.f) + (b->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[0].z/b->momentInertiaScalar:0.f));
		orthonormalBasisLambda[1] = dot(-relVel, cC->orthonormalBasis[1]) / (cC->contacts.orthonormalBasisInv[1].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[1].y/a->momentInertiaScalar:0.f) + (b->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[1].z/b->momentInertiaScalar:0.f));
		orthonormalBasisLambda[2] = dot(-relVel, cC->orthonormalBasis[2]) / (cC->contacts.orthonormalBasisInv[2].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[2].y/a->momentInertiaScalar:0.f) + (b->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[2].z/b->momentInertiaScalar:0.f));
		orthonormalBasisLambda[3] = (dot(-relVel, cC->orthonormalBasis[3]) + cC->contacts.bias) / (cC->contacts.orthonormalBasisInv[3].x + (a->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[3].y/ a->momentInertiaScalar :0.f) + (b->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[3].z/ b->momentInertiaScalar : 0.f));
		...
		for (int j(0); j < 4; j++) {
			...
			resolve_impulse(a, -impulses[j], cC->contacts.contactPosB);
			resolve_impulse(b, impulses[j], cC->contacts.contactPosA);
		}
	}
}
...
static void solve(std::vector <Constraint*>* cs, RigidBody4D* a, bool isInverseResolve)
{
	for (unsigned int i(0); i < cs->size(); i++)
	{
		...
		glm::vec4 newImpulses;
		glm::vec4 orthonormalBasisLambda;
		orthonormalBasisLambda[0] = dot(-relVel, cC->orthonormalBasis[0]) / (cC->contacts.orthonormalBasisInv[0].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[0].y/a->momentInertiaScalar:0.f) + cC->contacts.orthonormalBasisInv[0].z);
		orthonormalBasisLambda[1] = dot(-relVel, cC->orthonormalBasis[1]) / (cC->contacts.orthonormalBasisInv[1].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[1].y/a->momentInertiaScalar:0.f) + cC->contacts.orthonormalBasisInv[1].z);
		orthonormalBasisLambda[2] = dot(-relVel, cC->orthonormalBasis[2]) / (cC->contacts.orthonormalBasisInv[2].x + (a->momentInertiaScalar>0.f?cC->contacts.orthonormalBasisInv[2].y/a->momentInertiaScalar:0.f) + cC->contacts.orthonormalBasisInv[2].z);
		orthonormalBasisLambda[3] = (dot(-relVel, cC->orthonormalBasis[3]) + cC->contacts.bias) / (cC->contacts.orthonormalBasisInv[3].x + (a->momentInertiaScalar > 0.f ? cC->contacts.orthonormalBasisInv[3].y/ a->momentInertiaScalar :0.f) + cC->contacts.orthonormalBasisInv[3].z);
		newImpulses[0] = cC->contacts.orthonormalBasisImpulse[0] + orthonormalBasisLambda[0];
		newImpulses[1] = cC->contacts.orthonormalBasisImpulse[1] + orthonormalBasisLambda[1];
		newImpulses[2] = cC->contacts.orthonormalBasisImpulse[2] + orthonormalBasisLambda[2];
		newImpulses[3] = alg::max(cC->contacts.orthonormalBasisImpulse[3] + orthonormalBasisLambda[3], 0.f);
	...
	std::vector <std::vector<Constraint*>>* constraintsVec)
{
	if (collisions[0].collisionManifold.depth > 0.f)
	{
		...
		constraints.push_back(new Constraint(collision.index1, collision.index2,collisionConstraint(collision.collisionManifold, a, collision.massAdjustmentA, collision.type == 0 ? a->mu:INFINITY, b, collision.massAdjustmentB, collision.type == 0 ? b->mu:INFINITY)));
...
{
	solve(constraint, b, a, isInverseResolve);
}
	...
	std::vector <std::vector<Constraint*>>* constraintsVec)
{
	if (collisions[0].collisionManifold.depth > 0.f)
	{
		std::vector<Constraint*> constraints;
		for (Collision collision : collisions)
			constraints.push_back(new Constraint(collision.index1, collision.index2, collisionConstraint(collision.collisionManifold, a, collision.massAdjustmentA, collision.type == 0 ? a->mu : INFINITY,collision.type == 0 ? b->mu:INFINITY , b->restitution)));
		constraintsVec->push_back(constraints);
		for (Constraint* c : constraints)c = nullptr, free(c);
	}
}
	...
	std::vector <std::vector<Constraint*>>* constraintsVec)
{
	if (collisions[0].collisionManifold.depth > 0.f)
	{
		std::vector<Constraint*> constraints;
		for (Collision collision : collisions)
		constraints.push_back(new Constraint(collision.index1, -1, collisionConstraint(collision.collisionManifold, a, collision.massAdjustmentA, collision.type == 0 ? a->mu : INFINITY, collision.type == 0 ? 0.7f:INFINITY, 0.25f)));
		constraintsVec->push_back(constraints);
		for (Constraint* c : constraints)c = nullptr, free(c);
	}
}
