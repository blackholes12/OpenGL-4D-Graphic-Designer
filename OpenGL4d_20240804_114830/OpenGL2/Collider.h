...
void Game::updateObjects()
{
	...
	if (this->isFixedTime)
	{
		phy::update_physic4D(this->rigidBodies4D,this->dynamites4D2, this->groups,this->walls4D,this->dynamites4D, this->waters4D,&this->collisionsVec,&this->constraintsVec, &this->constraintsVec2, this->sticks4D, this->springs4D,this->hingeConstraintsVec, 1 / 60.f);
	}
...
void Game::updateCollider() 
{
	if (this->isFixedTime)
	{
		...
		{
			if (collisions[0].index1 == 0)
			{
				if (collisions[0].collisionManifold.normal.y > 0.5f)
				{
					this->camera.isGround = true;
					this->rigidBodies4D[0]->isGravity = true;
				}
				if (collisions[0].index2 >= 0)
				if (find_object4d(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D, this->dynamites4D, collisions[0].index2)->objectName == "Portal")
				{
			...
			if (find_dynamite4d(this->dynamites4D, this->dynamites4D2, i)->objectName == "Exploding Dynamite")
			{
				...
				for (size_t j(1); j < this->rigidBodies4D.size(); j++)
				{
					if (this->rigidBodies4D[j]->objectName == "Can-Be-Destroyed"||this->rigidBodies4D[j]->objectName == "Glass4D")
					{
						if (length(this->rigidBodies4D[j]->position4D - find_dynamite4d(this->dynamites4D, this->dynamites4D2, i)->position4D) < 3.f)
						{
							...
							erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, j);
							shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, j,-1);
				...
				for (unsigned j(0); j < size_of_walls4d(this->groups); j++)
				{
					if (find_wall4d(this->groups,j)->objectName == "Can-Be-Destroyed"|| find_wall4d(this->groups, j)->objectName == "Glass4D")
					{
						...
						{
							erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, j + size_of_objects4d(this->rigidBodies4D,this->dynamites4D2));
							shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, j + size_of_objects4d(this->rigidBodies4D, this->dynamites4D2), -1);
				...
				{
					erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, size_of_objects4d(this->rigidBodies4D, this->dynamites4D2,this->groups) + i);
					shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, size_of_objects4d(this->rigidBodies4D, this->dynamites4D2,this->groups) + i,-1);
				}
				if (i >= this->dynamites4D.size())
				{
					...
					erase_joint4d(&this->sticks4D, &this->springs4D,&this->hingeConstraintsVec, this->rigidBodies4D.size() + i - this->dynamites4D.size());
					shift_joint4d(this->sticks4D, this->springs4D, &this->hingeConstraintsVec, this->rigidBodies4D.size() + i - this->dynamites4D.size(),-1);
		...
		for (size_t i(0); i < this->emitters4D.size(); i++) {
			...
			if (this->emitters4D[i]->type == 0)
			{
				...
				if (emitTime00 / (float)emitCount00 >= 1.f / 60.f)
				{
					...
					{
						this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE2, this->emitters4D[i]->position4D, 8.f * normalize(glm::vec4(rand() % 100 - 50, rand() % 160, rand() % 100 - 50, 0.f)), 1.f, 1.f, glm::vec4(1.f, 0.f, 0.f, 1.f), glm::vec4(1.f, 0.f, 0.f, 1.f), true, 1.f, 1.f, 0.5f, 1.f, 1, 0));
				...
				else
				{
					if (this->emitters4D[i]->time == 0.f)
					{
						...
						{
							this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->position4D, this->emitters4D[i]->velocity4D / 10.f + (float)(rand() % 3 + 6) * normalize(glm::vec4(rand() % 100 - 50, rand() % 75 + 30, rand() % 100 - 50, rand() % 100 - 50)), 1.f / 5.f, 1.f / 5.f, glm::vec4(1.f, 0.f, 0.f, 1.f), glm::vec4(1.f, 0.f, 0.f, 1.f), true, 0.5f, 1.f, 0.9f, 0.4f, 1, 0));
						}
					}
				}
				...
				if (this->emitters4D[i]->time == 0.f)
				{
					for (int k(0); k < 6; k++)
					{
						this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->position4D + (float)(rand() % 3 + 5) / 15.5f * normalize(glm::vec4(rand() % 100 - 50))
					...
					for (int k(0); k < 4; k++)
					{
						this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->position4D + (float)(rand() % 3 + 5) / 17.5f * normalize(glm::vec4(rand() % 100 - 50))
				...
				for (size_t j(0); j < this->emitters4D[i]->particles4D.size(); j++) {
					...
					if (this->emitters4D[i]->particles4D[j]->level == 1 && this->emitters4D[i]->particles4D[j]->index == 0)
					{
						if (this->emitters4D[i]->particles4D[j]->emitTime >= this->emitters4D[i]->particles4D[j]->duration / (float)emitCount1)
						{
							...
							this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->particles4D[j]->position4D+0.2f*normalize(glm::vec4(rand()%100-50, rand() % 100 - 50, rand() % 100 - 50, rand() % 100 - 50)), this->emitters4D[i]->particles4D[j]->velocity4D * this->emitters4D[i]->particles4D[j]->retention
					...
					if (this->emitters4D[i]->particles4D[j]->level == 2 && this->emitters4D[i]->particles4D[j]->index == 0)
					{
						...
						if (this->emitters4D[i]->particles4D[j]->emitTime >= this->emitters4D[i]->particles4D[j]->duration / (float)emitCount2 * timeShift)
						{
							...
							this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->particles4D[j]->position4D, this->emitters4D[i]->particles4D[j]->velocity4D * this->emitters4D[i]->particles4D[j]->retention
			...
			if (this->emitters4D[i]->type == 1)
			{
				...
				if (emitTime00 / (float)emitCount00 >= 1.f / 60.f)
				{
					if ((this->emitters4D[i]->emitTime >= emitTime00 / (float)(emitCount00 - 1) && this->emitters4D[i]->time <= emitTime00) ||
						this->emitters4D[i]->time == 0.f)
					{
						this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE2, this->emitters4D[i]->position4D, 8.f * normalize(glm::vec4(rand() % 100 - 50, rand() % 160, rand() % 100 - 50, 0.f)), 1.f, 1.f, glm::vec4(1.f, 0.65f, 0.12f, 1.f), glm::vec4(1.f, 0.65f, 0.12f, 0.f), true, 1.f, 1.f, 0.5f, 1.f, 1, 0));
						this->emitters4D[i]->emitTime = 0.f;
					}
				}
				else
				{
					if (this->emitters4D[i]->time == 0.f)
					{
						for (int k(0); k < emitCount00; k++)
						{
							this->emitters4D[i]->particles4D.push_back(new Particle4D(PARTICLE3, this->emitters4D[i]->position4D, this->emitters4D[i]->velocity4D / 10.f + 0.5f*(float)(rand() % 3 + 6) * normalize(glm::vec4(rand() % 100 - 50, rand() % 75 + 30, rand() % 100 - 50, rand() % 100 - 50)), 1.f / 5.f, 1.f / 5.f, glm::vec4(0.35f, 0.35f, 0.35f, 1.f), glm::vec4(0.15f, 0.15f, 0.15f, 0.f), false, 3.f/18.f, 1.f, 0.5f, 0.4f, 1, 0));
						}
					}
				}
				//
