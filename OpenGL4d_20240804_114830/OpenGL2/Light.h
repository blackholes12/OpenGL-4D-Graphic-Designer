...
public:
	...
	void set_light4D_property(Shader* type, unsigned id)
	{
		...
		type->setVec3f(this->color, ss);
		snprintf(ss, 255, "pointLight4D[%i].radius", id);
...
public:
	...
	float get_cloud_distance(glm::vec4 ray, float radious, float height)
	{
	...
	void update_light4D_in_earth_environment(const float period,const float worldTime)
	{
		...
		{
			float distance = 1.225f*65.68501f* get_cloud_distance(-this->direction4D,6371000.f,8228.f)/ 6371000.f;
			this->color = 1.1f*0.96396755751f *glm::vec3(exp(-0.1889f * distance), exp(-0.51f * distance), exp(-1.2576f * distance));
		}
		else
		{
	...
	void set_light4D_property(Shader* shader)
	{
		...
		shader->use();
		shader->setVec4f(this->direction4D, "directionLight4D.direction4D");
		shader->set1f(this->intensity, "directionLight4D.intensity");
		shader->setVec3f(this->color, "directionLight4D.color");
	}
