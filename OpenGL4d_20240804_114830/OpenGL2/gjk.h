    ...
    glm::vec4 nearest_simplex()
    {
        ...
        case 5:
        {
            ...
            for (int i(0); i < 4; i++) {
                ...
                if (aoDist * dot(e - a, normal) < 0.00001f) {
...
static bool gjk(Object4D* s1, Object4D* s2, Simplex* simplex)
{
    glm::vec4 a(support_of_cso(s1, s2, s1->position4D-s2->position4D) + glm::vec4(10000 + rand() % 100, 10000 + rand() % 100, 10000 + rand() % 100, 10000 + rand() % 100) * 0.00000009f);
    *simplex = Simplex(a);
    glm::vec4 d(-a);
    ...
    while(true)
    {
        ...
        simplex->push(a + glm::vec4(10000 + rand() % 100, 10000 + rand() % 100, 10000 + rand() % 100, 10000 + rand() % 100) * 0.00000009f);
...
struct ExpandingPolytope2
{
    ...
    void expand(glm::vec4 newPoint4D)
    {
        ...
        for (size_t i(0); i < newTetrasIndexs.size(); i++)
        {
        ...
        for (size_t i(0); i < newTetrasIndexs.size(); i++)
        {
            for (size_t j(i + 1); j < newTetrasIndexs.size(); j++)
            {
...
static glm::vec4 epa2(Object4D* s1, Object4D* s2, Simplex simplex, float* depth,bool isCapsule)
{
    unsigned int count(0);
    ...
    while (true)
    {
        ...
        if (dot(expandPoint, normal) - distance < 0.0001f)
        {
            break;
        }
        if (count > 24)
