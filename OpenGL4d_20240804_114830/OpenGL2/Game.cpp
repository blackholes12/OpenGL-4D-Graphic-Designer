...
void Game::startShaders()
{
	...
	this->shaderHDRs.push_back(new Shader(this->GL_VERSION_MAJOR, this->GL_VERSION_MINOR,
		"resourcefile/shaders/hdr.vertex", "resourcefile/shaders/bloom_mipmap.fragment"));
	this->shaderHDRs.push_back(new Shader(this->GL_VERSION_MAJOR, this->GL_VERSION_MINOR,
...
void Game::startTextures()
{
	...
	{
		this->bloomMipMaps.push_back(new HDRTexture(this->framebufferWidth / pow(2, i) / 3, this->framebufferHeight / pow(2, i) / 3));
		this->bloomMipMaps[i]->set_filter(1);
	}
...
void Game::startDemos()
{
	...
	this->demos4D.push_back(new Demo4D(new Spring4D(glm::vec4(0.f), -1, -1, this->springLength, this->springStrength, STONE, this->buildColor, DEMOD4)));
}
...
void Game::updateProjectMat(int fbW, int fbH) {
	...
	for (size_t i(0); i < 8; i++){
		this->bloomMipMaps[i]->set_texture_size(this->framebufferWidth / pow(2, i) / 3, this->framebufferHeight / pow(2, i) / 3);
	}
	...
	camera(60.f, static_cast<float>(this->framebufferWidth) / this->framebufferHeight, 0.1f)
{
	...
	this->startDefault();
	this->startInputs();
	this->skybox4D = new Sky_Box4D("Sky4d", Box4d(glm::vec4(1.f)), alg::bivec4(), glm::vec4(100000.f), SKY_BOXD4);
...
Game::~Game()
{
	...
	for (HDRTexture* i : this->bloomMipMaps) { i = nullptr, delete i; }
	...
	for (std::vector<Constraint*> i : constraintsVec2)for (Constraint* j : i) { j = nullptr, delete j; }
		...
		//if (fixedUpdate::is_fixed_update2(this->dt, 1.f / 30.f))
		{
			...
			if (this->isFindPoint) {
				...
				case BUILD_OBJECT:
				{
					...
					this->demoObjects4D->walls4D[0]->set_position4D(hitObject4D == nullptr ? get_cap_position4D(this->hittingPos4D + this->buildScale.y * this->hitNormal / 2.f) : get_cap_position4D(hitObject4D, isFindPoint, this->hittingPos4D + this->buildScale.y * this->hitNormal / 2.f));
					...
					if (this->hitIndex >= size_of_objects4d(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D) || this->hitIndex < size_of_objects4d(this->rigidBodies4D, this->dynamites4D2, this->groups))
					{
						...
						if (isInitial)
						{
							...
							else if (this->sticks4D.size() != 0)
							{
								...
								else if (this->sticks4D[this->sticks4D.size() - 1]->index2 != -1)
								{
									this->demoObjects4D->stick4D = this->demos4D[30]->stick4D;
									this->demoObjects4D->stick4D->fixPoint4D = this->camera.frontPosition4D;
									this->demoObjects4D->stick4D->index1 = this->hitIndex;
					...
					if (this->hitIndex >= size_of_objects4d(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D) || this->hitIndex < size_of_objects4d(this->rigidBodies4D, this->dynamites4D2, this->groups))
					{
						...
						if (isInitial)
						{
							...
							else if (this->springs4D.size() != 0)
							{
								...
								else if (this->springs4D[this->springs4D.size() - 1]->index2 != -1)
								{
									this->demoObjects4D->spring4D = this->demos4D[31]->spring4D;
									this->demoObjects4D->spring4D->fixPoint4D = this->camera.frontPosition4D;
									this->demoObjects4D->spring4D->index1 = this->hitIndex;
...
void Game::render0()
{
	...
	if (ray::is_in_screen(-this->directionLight4D->direction4D, camera.viewMat4D, camera.projectionMat))
	{
		...
		if (ray::detect_object4d_index(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D, this->dynamites4D, this->sticks4D, this->springs4D, true, true, false, true, this->camera.position4D, -this->directionLight4D->direction4D, this->directionLight4D->right4D, &hitPosNear2,&hitPosNormal2) == -1)
		{
			glDepthMask(false);
	...
	if (this->isHDR)
	{
		...
		if (this->isBloom)
		{
			this->bloomMipMaps[0]->bind_frame_buffer();
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			...
			this->bloomMipMaps[0]->unbind_frame_buffer();

			for (size_t i(1); i < 8; i++)
			{
				this->bloomMipMaps[i]->bind_frame_buffer();
				glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
				float texScale(3 * pow(2, i));
				glViewport(0, 0, this->framebufferWidth / texScale, this->framebufferHeight / texScale);
				this->hdrQuad->render(this->bloomMipMaps[i-1], this->shaderHDRs[MIP], glm::vec2(2*this->framebufferWidth / texScale, 2*this->framebufferHeight / texScale));
				this->bloomMipMaps[i]->unbind_frame_buffer();
			}

			this->hDRTextures[1]->bind_frame_buffer();
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			glViewport(0, 0, this->framebufferWidth / 3, this->framebufferHeight / 3);
			this->hdrQuad->render(this->bloomMipMaps, this->shaderHDRs[BLOOM], glm::vec2(this->framebufferWidth / 3, this->framebufferHeight / 3));
		...
		//std::cout << "brightness=" << this->exposure << "\n";
	}
	//6:GUI
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glLineWidth(1.9f);
