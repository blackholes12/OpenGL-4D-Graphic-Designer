	...
	else
	{
		...
		while (std::getline(in_file, line2))
		{
			...
			if (prefix2 == 4) //terrain
			{
				...
				terrains4D->push_back(new Terrain4D(name_list(nameIndex), Terrain4d(glm::ivec4(position4D.x, 54, position4D.z, position4D.w),8), position4D, scale4D, mu, bounce, colType, shaders[shader]));
			}
			if (prefix2 == 5) //water
			{
				unsigned nameIndex;
