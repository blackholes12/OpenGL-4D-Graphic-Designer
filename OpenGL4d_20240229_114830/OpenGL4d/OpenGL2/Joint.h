...
public:
	...
	float stickLength;
	Texture3D* diffuse3D;
	glm::vec4 paintingColorText;
	JointLine4D* jointLine4D;

	Stick4D(glm::vec4 fixPoint4D,int index1,int index2,float stickLength, Texture3D* diffuse3D, glm::vec4 paintingColorText, Shader* shader)
	: Object4D("Stick4D", Hypercylinder(glm::vec4(0.1f, 0.1f, stickLength, 0.1f)), fixPoint4D, alg::bivec4(), glm::vec4(0.1f,0.1f, stickLength,0.1f),0,0, 0, shader)
	{
		...
		this->paintingColorText = paintingColorText;
	...
	void update_stick4D(Object4D* a, Object4D* b)
	{
		...
		this->set_rotation_mat4(look_at_mat4(a->position4D, b->position4D));
	...
	void update_stick4D(Object4D* a)
	{
		...
		this->set_rotation_mat4(look_at_mat4(this->fixPoint4D, a->position4D));
		this->jointLine4D->update_joint_line4D(this->fixPoint4D, a->position4D);
	}
	~Stick4D(){
		delete this->jointLine4D;
	}
	...
	void render()
	{
		...
		this->shader->setVec4f(this->paintingColorText, "paintingColorText");
...
public:
	...
	Spring4D(glm::vec4 fixPoint4D, int index1, int index2,float springLength,float k, Texture3D* diffuse3D, glm::vec4 paintingColorText, Shader* shader)
		: Object4D("Spring4D", Hypercylinder(glm::vec4(0.05f, 0.05f, springLength, 0.05f)), fixPoint4D, alg::bivec4(), glm::vec4(0.05f, 0.05f, springLength, 0.05f),0,0, 0, shader)
	{
		this->fixPoint4D = fixPoint4D;
		this->index1 = index1;
		this->index2 = index2;
...
static struct HingeConstraint
{
	...
	explicit HingeConstraint(glm::vec4 worldPosition4D, glm::vec4 bodyPosition4D1, glm::vec4 bodyPosition4D2, unsigned index1,unsigned index2)
	{
