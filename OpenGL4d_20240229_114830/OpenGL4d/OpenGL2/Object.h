...
public:
	...
	Wall4D(const char* objectName,
		...
		unsigned int colType,
		Shader* shader)
		: Object4D(objectName,primitive4D,position4D,rotateAngle4D,scale4D,mu,bounce, colType,shader)
	{
...
static void update_water4d(std::vector<Water4D*> waters4D, Shader* type)
{
	if (waters4D.size() != 0)
	{
		char ss[256];
		...
		for (int i = 0; i < waters4D.size(); i++)
		{
			...
			type->setVec4f(waters4D[i]->position4D, ss);
			snprintf(ss, 255, "waterArea4D[%d].scale4D", i0);
			type->setVec4f(waters4D[i]->scale4D, ss);
			i0++;
		}
		type->set1i(i0, "D4_WaterAmount");
	}
	if (waters4D.size() == 0)
	{
		type->use();
		type->set1i(0, "D4_WaterAmount");
	}
}
...
public:
	Terrain4D(
		...
		unsigned int colType,
		Shader* shader
	) : Object4D(objectName, primitive4D, position4D, alg::bivec4(0, 0, 0, 0, 0, 0), scale4D, mu, bounce, colType, shader)
	{
	}
...
public:
	...
	void render(Shader* shader)
	{
		...
		shader->set1f(size*size*size, "intensity");
