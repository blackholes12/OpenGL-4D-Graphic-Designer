...
Game::~Game()
{
	...
	for (RenderIndex* i : this->renderIndices) { delete i; i = nullptr; }
	...
	for (Constraint* i : this->constraints) { delete i; i = nullptr; }
...
void Game::beforeRender()
{
	...
	if (this->judgetime0 > 0.13125f) {
		update_render_indices(this->rigidBodies4D, this->dynamites4D2, this->groups, this->walls4D, this->dynamites4D, &this->renderIndices, this->camera.position4D);
		sort_render_indices(&this->renderIndices);
		this->judgetime0 = 0.f;
	}
}
...
void Game::render0()
{
	...
	for (int index=0;index<this->renderIndices.size();index++)
	{
		RenderIndex* i = this->renderIndices[index];
		if (i->groupIndex > -1)
		{
			Wall4D* a = this->groups[i->groupIndex]->walls4D[i->objectIndex];
			if (!(a->diffuse3D->isTransparent || a->paintingColor.w < 1.f))
			{
				//a->paintingColor.w = 0.5f;
				a->render();
				//a->paintingColor.w = 1.f;
			}
		}
	...
	for (int index = this->renderIndices.size()-1; index >=0; index--)
	{
		RenderIndex* i = this->renderIndices[index];
		if (i->groupIndex > -1)
		{
