...
static glm::vec4 get_collision_information(Object4D* s1, Object4D* s2, unsigned steps, Simplex simplex, float* depth, std::vector<glm::vec4>* supportPoints1, std::vector<glm::vec4>* supportPoints2)
{
    ...
    while (true)
    {
        ...
        if (abs(dot(polytope.ePTetras[i2].normal4D, support_of_cso(s1, s2, polytope.ePTetras[i2].normal4D) - distance)) < 0.001f)
        {
            break;
        }
        polytope.expand(support_of_cso(s1, s2, polytope.ePTetras[i2].normal4D));
        count++;
        if (count > steps)
        {
            break;
