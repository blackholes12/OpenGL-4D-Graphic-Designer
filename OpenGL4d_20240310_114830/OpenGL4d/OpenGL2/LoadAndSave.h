...
static void save_file(const char* filePath, vector<Texture3D*> wallTextures3D, vector<Texture3D*> particleTextures3D, vector<Shader*> shaders, vector<RigidBody4D*> rigidBodies4D, vector<RigidBody4D*> dynamites4D2, std::vector<Group*> groups, vector<Wall4D*> walls4D, vector<Wall4D*> dynamites4D, vector < Water4D*> waters4D,
	...
	vector < PointLight4D*> pointLights4D)
{
	...
	for (int i = 1; i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++)
	{
		...
		std::cout <<type<< " " << r->objectName <<" " << primitive_index(r->primitive4D.primitiveName) <<" " <<
			...
			r->mass<<" " << r->momentInertiaScalar<<" " << r->mu << " "<<r->bounce << " "<<
		...
		while (std::getline(in_file, line2))
		{
			...
			if (prefix2 == 0|| prefix2 == 1) //rigid body
			{
				...
				ss2 >> objectName >> primitiveIndex >> primitiveScale4D.x >> primitiveScale4D.y >> primitiveScale4D.z >> primitiveScale4D.w >>
					...
					colType>> isTrail4D>> shader;
				const char* objectName2 = objectName.c_str();
				//std::cout << "///" << objectName2 << "///" << "\n";
				if (objectName == "Glass4D")
				{
					objectName2 = "Glass4D";
				}
				if (objectName == "Can Be Destroyed")
				{
					objectName2 = "Can Be Destroyed";
				}
				if(prefix2 == 0)
				    rigidBodies4D->push_back(new RigidBody4D(objectName2, primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mass, momentInertiaScalar, mu, bounce, isGravity, isFrozen, velocity4D, angularVelocity4D, isSpecular, wallTextures3D[diffuse3D], metal, paintingColor, colType, isTrail4D, shaders[shader]));
			    if(prefix2 == 1)
					dynamites4D2->push_back(new RigidBody4D(objectName2, primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mass, momentInertiaScalar, mu, bounce, isGravity, isFrozen, velocity4D, angularVelocity4D, isSpecular, wallTextures3D[diffuse3D], metal, paintingColor, colType, isTrail4D, shaders[shader]));
			}
			if (prefix2 == 2 || prefix2 == 3) //wall
			{
				...
				if (prefix2 == 2)
					walls4D->push_back(new Wall4D(objectName2, primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mu, bounce, isSpecular, wallTextures3D[diffuse3D], metal, paintingColor, colType, shaders[shader]));
				if (prefix2 == 3)
					dynamites4D->push_back(new Wall4D(objectName2, primitive_list(primitiveIndex, primitiveScale4D), position4D, rotation4D, scale4D, mu, bounce, isSpecular, wallTextures3D[diffuse3D], metal, paintingColor, colType, shaders[shader]));
			}
			if (prefix2 == 4) //terrain
			{
				...
				const char* objectName2 = objectName.c_str();
				terrains4D->push_back(new Terrain4D(objectName2, primitive_list(primitiveIndex, primitiveScale4D), position4D, scale4D,mu,bounce, colType, shaders[shader]));
			}
			if (prefix2 == 5) //water
			{
				std::string objectName;
				...
				waters4D->push_back(new Water4D(objectName.c_str(), primitive_list(primitiveIndex, primitiveScale4D), position4D, scale4D, colType, shaders[shader]));
			}
			if (prefix2 == 6)
			{
				glm::vec4 fixPoint4D;
