...
void Game::updateObjects()
{
	if (this->isFixedTime)
	{
		phy::update_physic4D(this->rigidBodies4D,this->dynamites4D2, this->groups,this->walls4D,this->dynamites4D, this->waters4D,&this->collisions, this->sticks4D, this->springs4D,this->hingeConstraints,&this->constraints, 1 / 60.f);
	}
			...
			if (find_dynamite4d(this->dynamites4D, this->dynamites4D2, i)->objectName == "Exploding Dynamite")
			{
				for (size_t j = 0; j < size_of_objects4d(this->rigidBodies4D,this->dynamites4D2); j++)
				{
					if (length(find_rigidbody4d(this->rigidBodies4D, this->dynamites4D2, j)->position4D - find_dynamite4d(this->dynamites4D, this->dynamites4D2, i)->position4D) < 10.f)
					{
						...
						find_rigidbody4d(this->rigidBodies4D, this->dynamites4D2, j)->forceSystems4D.push_back(new ForceSystem4D(3000.f * normalize(find_rigidbody4d(this->rigidBodies4D, this->dynamites4D2, j)->position4D - find_dynamite4d(this->dynamites4D, this->dynamites4D2, i)->position4D) / (distance * distance * distance), glm::vec4(0.f)));
					}
				}
...
void Game::updateParticles()
{
	if (this->isFixedTime)
	{
		for (unsigned i = 0; i < this->emitters4D.size(); i++) {
			//Emitter
			...
			for (unsigned j = 0; j < this->emitters4D[i]->particles4D.size(); j++) 
			{
