...
void Game::imgui_use() {
	...
	if (ImGui::TreeNodeEx("Levels", 2)) {
		...
		{
			if (ImGui::Selectable(("Level 1")))
			{
				this->startLevel(this->level = 1, 0, "");
				levelText = "Level 1";
			}
			if (ImGui::Selectable(("Level 2")))
			{
				this->startLevel(this->level = 2, 0, "");
				levelText = "Level 2";
			}
			if (ImGui::Selectable(("Level 3")))
			{
				this->startLevel(this->level = 3, 0, "");
				levelText = "Level 3";
			}
			if (ImGui::Selectable(("Level 4")))
			{
				this->startLevel(this->level = 4, 0, "");
				levelText = "Level 4";
			}
			if (ImGui::Selectable(("Level 5")))
			{
				this->startLevel(this->level = 5, 0, "");
				levelText = "Level 5";
			}
			if (ImGui::Selectable(("Level 6")))
			{
				this->startLevel(this->level = 6, 0, "");
				levelText = "Level 6";
			}
			ImGui::EndCombo();
		}
		if (ImGui::TreeNodeEx("Save", 0)) 
		{
			...
			if (ImGui::BeginCombo("Load The Level", ""))
			{
				...
				{
					this->startLevel(0, 1, entry.path().string().c_str());
		...
		//if (ImGui::TreeNodeEx("Saved Levels"), 0)
		//{
		//	
		//	ImGui::TreePop();
		//}
	...
	if (ImGui::TreeNodeEx("Controls", 2))
	{
		...
		{
			this->rigidBodies4D[0] = new RigidBody4D("Player", Hypercube(glm::vec4(0.4f, 1.4f, 0.4f, 0.4f)), glm::vec4(0.f, 64.7f, 0.f, 0.f), alg::bivec4(0.f, 0.f, 0.f, 90.f), glm::vec4(0.4f, 0.4f, 1.4f, 0.4f), 65.f, 0.f, 1.265f, 0.25f, true, false, glm::vec4(0, 0, 0, 0), alg::bivec4(), true, this->wallTexs3D[YELLOWMARBLE], 0.f, glm::vec4(1.f), CAPSULE4D, true, this->shaders[WALLD4]);
		}
	...
	if (ImGui::TreeNodeEx("Display", 2))
	{
		...
		if (ImGui::Checkbox("Display Trails 4D", &this->isTrail)) 
		{
			for (int i = 0; i < size_of_objects4d(rigidBodies4D, dynamites4D2); i++)
			{
				find_rigidbody4d(rigidBodies4D, dynamites4D2, i)->isTrail4D = this->isTrail;
			}
		}
		if(this->isTrail)
	...
	ImGui::Text("(%.1f xy %.1f xz %.1f xw %.1f yz %.1f yw %.1f zw) ¡ã Rotation", glm::degrees(this->camera.Rotate_Angle4D.xy), glm::degrees(this->camera.Rotate_Angle4D.xz), glm::degrees(this->camera.Rotate_Angle4D.xw), glm::degrees(this->camera.Rotate_Angle4D.yz), glm::degrees(this->camera.Rotate_Angle4D.yw), glm::degrees(this->camera.Rotate_Angle4D.zw));
	...
	if (ImGui::TreeNodeEx("Controls", 2))
	{
		ImGui::Text("Press W/S/A/D/R/F£ºplayers move forward, backward, left, right, ana, and kata");
		ImGui::Text("Space£ºjump");
