...
const char* cameraDir = "W";
const char* swapIntervalText = "vsyncI";
const char* BuildingCol = "BOX4D";
const char* BuildingShader = "Standard";
const char* PaintingColor = "R1/G1/B1/A1";
const char* PaintingShape = "Sphere";
const char* ObjectRendering = "Objects And Wire Frames";
const char* TextureFilter = "Mag:Linear/Min:Linear Mipmap Linear";
...
void Game::imgui_use() {
	...
	if (ImGui::TreeNodeEx("Level", 2)) {
		if (ImGui::BeginCombo("Level", ("Level " + to_string(this->level)).c_str()))
		{

			for (int levelCount = 1; levelCount <= 6; levelCount++)
			{
				if (ImGui::Selectable(("Level " + to_string(levelCount)).c_str()))
				{
					this->startLevel(this->level = levelCount);
				}
			}
			ImGui::EndCombo();
		}
	...
	if (ImGui::TreeNodeEx("Control", 2))
	{
		...
		if (ImGui::BeginCombo("Camera Vertical To", cameraDir))
		{
	...
	if (ImGui::TreeNodeEx("Ground", 2))
	{
		...
		if (ImGui::BeginCombo("Painting Shape", PaintingShape))
		{
		...
		if (ImGui::TreeNodeEx("Painting Texture", 1))
		{
			HelpMarker("(1) ","Sand");
			ImGui::SameLine();
	...
	if (ImGui::TreeNodeEx("Object", 2))
	{
		...
		{
			if (ImGui::TreeNodeEx("Object4D Texture", 1))
			{
				HelpMarker("(1) ", "GREEN");
				ImGui::SameLine();
				...
				ImGui::SameLine(); HelpMarker("(36)", "GRID");
				ImGui::SameLine();
				...
				ImGui::SameLine(); HelpMarker("(38)", "STARS_SKY");
				ImGui::SameLine();
				...
				ImGui::SameLine(); HelpMarker("(40)","CEIL2");
				ImGui::SameLine();
            ...
            if (ImGui::TreeNodeEx("Object4D Texture2", 1))
			{
			...
			if (ImGui::BeginCombo("Shader Type", BuildingShader))
			{
				if (ImGui::Selectable("Standard")) {
					this->buildShader = WALLD4;
					BuildingShader = "Standard";
				}
			...
			if (ImGui::BeginCombo("Building Collider", BuildingCol))
			{
				if (ImGui::Selectable("Sphere4D"))
				{
					this->buildCol = HYPERSPHERE;
					BuildingCol = "Sphere4D";
				}
			...
			ImGui::DragFloat4("Initial Velocity", &this->initialVelocity.x, 0.1f, 0.f, 20.f);
			...
			ImGui::Text("Rigid Body's (1)Mass");
			ImGui::Text("(2)Inertia Scalar");
			ImGui::Text("(3)Frictional Coefficient");
			ImGui::Text("(4)Bounce");
			...
			if (this->isIlluminate)
			{
				...
				ImGui::DragFloat4("Illuminate Scale", &this->lightScale.x, 0.1f, 0.f, 20.f);
			}
			...
			if (ImGui::ImageButton((ImTextureID)(BUILD_POOL + CEIL2 + SAND_SOIL2 + 5), ImVec2(100, 100)))
			{
				this->targetIcon->logo = this->pointerTexs[BUILDING];
				this->mode = BUILDING;

				this->buildMode = BUILD_POOL;
			...
			if (ImGui::Button("Delete Object In The Area"))
			{
				this->deleteMap3D->getImageData();
				{
					for (size_t index = 1; index < size_of_objects4d(this->rigidBodies4D, this->dynamites4D2); index++) {
						glm::ivec4 iobjectPos4D = glm::ivec4(floor(find_rigidbody4d(this->rigidBodies4D, this->dynamites4D2, index)->position4D));
						if (this->deleteMap3D->isDelete(glm::ivec3(iobjectPos4D.w + 64, iobjectPos4D.z + 64, iobjectPos4D.x + 64)))
						{
							if (find_rigidbody4d(this->rigidBodies4D, this->dynamites4D2, index - 1)->objectName != "Ground")
							{
								//std::cout << "ImageData=" << iobjectPos4D.x + 64<< iobjectPos4D.z + 64<< iobjectPos4D.w + 64 << "\n";
				...
				{
					for (size_t index = 0;index < i->walls4D.size();index++) {
						glm::ivec4 iobjectPos4D = glm::ivec4(floor(i->walls4D[index]->position4D));
						index++;
						if (this->deleteMap3D->isDelete(glm::ivec3(iobjectPos4D.w + 64, iobjectPos4D.z + 64, iobjectPos4D.x + 64)))
						{
							if (i->walls4D[index]->objectName != "Ground")
							{
								//std::cout << "ImageData=" << iobjectPos4D.x + 64<< iobjectPos4D.z + 64<< iobjectPos4D.w + 64 << "\n";
								...
								i->walls4D.erase(i->walls4D.begin() + index);
								index--;
							}
						}
					}
...

				{
					for (size_t index = 0; index < this->walls4D.size(); index++) {
						glm::ivec4 iobjectPos4D = glm::ivec4(floor(this->walls4D[index]->position4D));
						if (this->deleteMap3D->isDelete(glm::ivec3(iobjectPos4D.w + 64, iobjectPos4D.z + 64, iobjectPos4D.x + 64)))
						{
				...
				
				{
					for (size_t index = 0; index < this->dynamites4D.size(); index++) {
						glm::ivec4 iobjectPos4D = glm::ivec4(floor(this->dynamites4D[index]->position4D));
						if (this->deleteMap3D->isDelete(glm::ivec3(iobjectPos4D.w + 64, iobjectPos4D.z + 64, iobjectPos4D.x + 64)))
						{
				...
				}
				{
					for (size_t index = 0; index < this->pointLights4D.size(); index++) {
						glm::ivec4 iobjectPos4D = glm::ivec4(floor(this->pointLights4D[index]->position4D));
						if (this->deleteMap3D->isDelete(glm::ivec3(iobjectPos4D.w + 64, iobjectPos4D.z + 64, iobjectPos4D.x + 64)))
						{
	...
	if (ImGui::BeginCombo("Objects Rendering", ObjectRendering))
	{
		...
		if (ImGui::Selectable("Objects And Wire Frames"))
		{
			this->frameType = 1;
	...
	if (ImGui::BeginCombo("Texture Filter", TextureFilter))
	{
		if (ImGui::Selectable("Mag:Nearest/Min:Nearest"))
		{
			this->filterType = 0;
			TextureFilter = "Mag:Nearest/Min:Nearest";
		...
		{
			this->filterType = 1;
			TextureFilter = "Mag:Nearest/Min:Nearest Mipmap Linear";
			for (Texture3D* i : this->textures3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->particleTexs3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->wallTexs3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->groundTexs3D)i->set_filter(this->filterType);
		...
		{
			this->filterType = 2;
			TextureFilter = "Mag:Linear/Min:Linear";
			for (Texture3D* i : this->textures3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->particleTexs3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->wallTexs3D)i->set_filter(this->filterType);
			for (Texture3D* i : this->groundTexs3D)i->set_filter(this->filterType);
	...
	ImGui::Text("(%.1f xy %.1f xz %.1f xw %.1f yz %.1f yw %.1f zw) бу Rotation", glm::degrees(this->camera.Rotate_Angle4D.xy), glm::degrees(this->camera.Rotate_Angle4D.xz), glm::degrees(this->camera.Rotate_Angle4D.xw), glm::degrees(this->camera.Rotate_Angle4D.yz), glm::degrees(this->camera.Rotate_Angle4D.yw), glm::degrees(this->camera.Rotate_Angle4D.zw));
	...
	ImGui::Spacing();
	ImGui::Text("Other:");
	ImGui::Text("%1i Dimension level", this->level);
	...
	ImGui::Text("(%1i m) Block (%1i m) Object (%1i m) Light", this->blockLoadDistance, this->objectLoadDistance, this->lightLoadDistance);
