	...
	*/
	...
	glm::vec4* hitPosNear)
	{
		...
		for (size_t i = 1; i < rigidBodies4D.size(); i++) {
			...
			{
				if (ray::ray_detect_collider4D(rigidBodies4D[i], camPos, camFor, camRight, hitPosNear, &hitPosFar))
				{
		...
		for (size_t i = 0; i < dynamites4D2.size(); i++) {
			...
			if (cosView * distance4D > -colRadious && sinView * distance4D < colRadious)
			{
				if (ray::ray_detect_collider4D(dynamites4D2[i], camPos, camFor, camRight, hitPosNear, &hitPosFar))
				{
					if (length(*hitPosNear - camPos) < length(detectedObjectPos - camPos))
					{
						detectedObjectPos = *hitPosNear;
		...
		if (isDetectStatic)
		{
			for (size_t i = 0; i < size_of_walls4d(groups); i++) {
				if (is_wall4d_being_rendered(groups, i))
				{
					if (!find_wall4d(groups, i)->diffuse3D->isTransparent || isDetectTransparent)
					{
						...
						if (cosView * distance4D > -colRadious && sinView * distance4D < colRadious)
						{
							if (ray::ray_detect_collider4D(find_wall4d(groups, i), camPos, camFor, camRight, hitPosNear, &hitPosFar))
							{
								if (length(*hitPosNear - camPos) < length(detectedObjectPos - camPos))
								{
									detectedObjectPos = *hitPosNear;
			...
			for (size_t i = 0; i < walls4D.size(); i++) {
				...
				if (cosView * distance4D > -colRadious && sinView * distance4D < colRadious)
				{
					if (ray::ray_detect_collider4D(walls4D[i], camPos, camFor, camRight, hitPosNear, &hitPosFar))
					{
						if (length(*hitPosNear - camPos) < length(detectedObjectPos - camPos))
						{
							detectedObjectPos = *hitPosNear;
			...
			for (size_t i = 0; i < dynamites4D.size(); i++) {
				...
				if (cosView * distance4D > -colRadious && sinView * distance4D < colRadious)
				{
					if (ray::ray_detect_collider4D(dynamites4D[i], camPos, camFor, camRight, hitPosNear, &hitPosFar))
					{
						if (length(*hitPosNear - camPos) < length(detectedObjectPos - camPos))
						{
							detectedObjectPos = *hitPosNear;
			...
			if (isDetectTerrain4D)
			{
				for (size_t i = 0; i < size_of_terrains4d(groups); i++)
				{
					...
					{
						if (ray::ray_detect_collider4D(find_terrain4d(groups, i), camPos, camFor, camRight, hitPosNear, &hitPosFar))
						{
							if (length(*hitPosNear - camPos) < length(detectedObjectPos - camPos))
							{
								detectedObjectPos = *hitPosNear;
