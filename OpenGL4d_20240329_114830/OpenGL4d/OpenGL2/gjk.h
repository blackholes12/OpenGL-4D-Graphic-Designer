...
static glm::vec4 support(Object4D* a,glm::vec4 direction4D)
{
    ...
    case BOX4D:
    {
        ...
        float dot0=0.f;
        {
            glm::vec4 vertexPos4D = hypercube::vertices4D[0] * a->scale4D;
            dot0 = dot(direction4DToObj, vertexPos4D);
    ...
    case MESH4D:
    {
        ...
        float dot0 = 0.f;
        {
            glm::vec4 vertexPos4D = a->primitive4D.vertexData4D[0] * a->scale4D;
            dot0 = dot(direction4DToObj, vertexPos4D);
            supportPosition4d = vertexPos4D;
        }
        for (int i = 1; i < a->primitive4D.get_size_of_vertices4D(); i++)
        {
    ...
    case TERRAIN4D:
    {
        ...
        float dot0 = 0.f;
        {
            glm::vec4 vertexPos4D = a->primitive4D.vertexData4D[0];
            dot0 = dot(direction4DToObj, vertexPos4D);
            supportPosition4d = vertexPos4D;
        }
   ...
   // on the updated simplex to the origin.
    glm::vec4 nearest_simplex()
    {
        ...
        case 5:
        {
            ...
            unsigned int i2 = -1;
            glm::vec4 direction4D2 = glm::vec4(10000.f);
            float aoDist2 = 0.f;
            for (int i = 0; i < 4; i++) {
                ...
                if (aoDist * dot(e - a, normal) < 0.000001f) {
...
{
    glm::vec4 a = support_of_cso(s1, s2, normalize(s2->position4D - s1->position4D+glm::vec4(1.f,2.f,4.f,8.f)));
    *simplex = Simplex(a);
    glm::vec4 d = -a;
...
struct ExpandingPolytope2
{
    ...
    void from_simplex(Simplex simplex)
    {
        ...
        else
        {
            ...
            std::cout << "length of simplex's vertex:" << simplex.length0 << "\n";
            this->vertices4D.push_back(0.01f * glm::vec4(0.158113883008419f, 0.204124145231932f, 0.288675134594813f, 0.5));
            this->vertices4D.push_back(0.01f * glm::vec4(0.158113883008419f, 0.204124145231932f, 0.288675134594813f, -0.5));
            this->vertices4D.push_back(0.01f * glm::vec4(0.158113883008419f, 0.204124145231932f, -0.577350269189626f, 0));
            this->vertices4D.push_back(0.01f * glm::vec4(0.158113883008419f, -0.612372435695794f, 0, 0));
            this->vertices4D.push_back(0.01f * glm::vec4(-0.632455532033676f, 0, 0, 0));
        }
        ...
        for (int i = 0; i < 5; i++)
        {
            glm::vec4 normal4D = normalize(cross4d(
            this->vertices4D[this->ePTetras[i].tetra4DIndexs[3]] - this->vertices4D[this->ePTetras[i].tetra4DIndexs[0]],
            this->vertices4D[this->ePTetras[i].tetra4DIndexs[2]] - this->vertices4D[this->ePTetras[i].tetra4DIndexs[0]],
            this->vertices4D[this->ePTetras[i].tetra4DIndexs[1]] - this->vertices4D[this->ePTetras[i].tetra4DIndexs[0]]));
    ...
    {
        std::vector<glm::ivec4> addTetrasIndexs;
        bool isRemove = false;
        unsigned addVertexIndex;
        ...
        if (!isRemove)
        {
            ...
            addVertexIndex = this->vertices4D.size() - 1;
            for (int i = 0; i < this->ePTetras.size(); i++)
            {
                if (dot(this->ePTetras[i].normal4D, this->vertices4D[addVertexIndex] - this->vertices4D[this->ePTetras[i].tetra4DIndexs[0]]) > 0.f)
                {
                    addTetrasIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[2], addVertexIndex));
                    addTetrasIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[3], addVertexIndex));
                    addTetrasIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[0], ePTetras[i].tetra4DIndexs[2], ePTetras[i].tetra4DIndexs[3], addVertexIndex));
                    addTetrasIndexs.push_back(glm::ivec4(ePTetras[i].tetra4DIndexs[1], ePTetras[i].tetra4DIndexs[2], ePTetras[i].tetra4DIndexs[3], addVertexIndex));
                    this->ePTetras.erase(this->ePTetras.begin() + i);
                    i--;
                }
            }
            for (int i = 0; i < addTetrasIndexs.size(); i++)
            {
                sort(addTetrasIndexs[i][0], addTetrasIndexs[i][1], addTetrasIndexs[i][2], addTetrasIndexs[i][3], &addTetrasIndexs[i][0], &addTetrasIndexs[i][1], &addTetrasIndexs[i][2], &addTetrasIndexs[i][3]);
            }
            for (int i = 0; i < addTetrasIndexs.size(); i++)
            {
                for (int j = i + 1; j < addTetrasIndexs.size(); j++)
                {
                    if (addTetrasIndexs[i] == addTetrasIndexs[j])
                    {
                        addTetrasIndexs.erase(addTetrasIndexs.begin() + j);
                        j--;
                        addTetrasIndexs.erase(addTetrasIndexs.begin() + i);
                        i--;
                    }
                }
            }
            //for (int i = 0; i < this->ePTetras.size(); i++)
            //{
            //    for (int j = 0; j < addTetrasIndexs.size(); j++)
            //    {
            //        if (this->ePTetras[i].tetra4DIndexs == addTetrasIndexs[j])
            //        {
            //            this->ePTetras.erase(this->ePTetras.begin() + i);
            //            i--;
            //            addTetrasIndexs.erase(addTetrasIndexs.begin() + j);
            ...
            unsigned int size = addTetrasIndexs.size();
            unsigned int pSizeOfTetras = this->ePTetras.size();
            this->ePTetras.resize(pSizeOfTetras + size);
            for (int i = 0; i < size; i++)
            {
                this->ePTetras[pSizeOfTetras + i].tetra4DIndexs = addTetrasIndexs[i];
                glm::vec4 normal4D = normalize(cross4d(
                    this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[3]] - this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[0]],
                    this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[2]] - this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[0]],
                    this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[1]] - this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[0]]));
                float distance = dot(normal4D, this->vertices4D[this->ePTetras[pSizeOfTetras + i].tetra4DIndexs[0]]);
                if (distance < 0.f)
                {
                    normal4D = -normal4D;
                    distance = -distance;
                }
                this->ePTetras[pSizeOfTetras + i].normal4D = normal4D;
                this->ePTetras[pSizeOfTetras + i].distance = distance;
            }
        }
    }
};

static glm::vec4 get_collision_information(Object4D* s1, Object4D* s2, unsigned steps, Simplex simplex, float* depth, std::vector<glm::vec4>* supportPoints1, std::vector<glm::vec4>* supportPoints2)
{
    unsigned int count = 0;
    ...
    while (true)
    {
        ...
        glm::vec4 expandPoint = support_of_cso(s1, s2, polytope.ePTetras[i2].normal4D);
        if (dot(polytope.ePTetras[i2].normal4D, expandPoint) - distance < 0.000001f)
        {
            break;
        }
    ...
    for (int i = 0; i < polytope.ePTetras.size(); i++)
    {
        glm::vec4 normal2 = polytope.ePTetras[i].normal4D;
        float depth2 = polytope.ePTetras[i].distance;
        if (i==0 || (depth0 > depth2 && depth2>0.f))
        {
            depth0 = depth2;
            i1 = i;
            normal3 = normal2;
    ...
    {
        *depth = depth0;
        *supportPoints1 = support_points(s1, normal3);
...
static glm::vec4 epa(Object4D* s1, Object4D* s2, unsigned steps, Simplex simplex, float* depth, std::vector<glm::vec4>* supportPoints1, std::vector<glm::vec4>* supportPoints2)
{
    ...
    while (true)
    {
        float distance = 0.f;
        unsigned index = 0;
        for (int i = 0; i < polytope.cells.size(); i++)
        {
            if (i == 0 || distance > polytope.cells[i].distance)
            {
                distance = polytope.cells[i].distance;
                index = i;
            }
        }
        EPCell minCell = polytope.cells[index];
        glm::vec4 normal = minCell.normal;
        glm::vec4 extendPoint = support_of_cso(s1, s2, normal);
        if (dot(extendPoint, normal) - minCell.distance > 0.001f)
        {
            polytope.expand(extendPoint);
            //std::cout << dot(extendPoint, normal) - minCell.distance << "\n";
        }
        else
        {
            normal4D = normal;
    ...
    for (int i = 0; i < polytope.cells.size(); i++)
    {
        glm::vec4 normal2 = polytope.cells[i].normal;
        float depth2 = dot(normal2, support_of_cso(s1, s2, normal2));
        if (i == 0 || (depth0 > depth2 && depth2 > 0.f))
        {
            depth0 = depth2;
            i1 = i;
