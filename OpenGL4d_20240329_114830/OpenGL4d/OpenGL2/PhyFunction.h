...
static void solve(CollisionConstraint c, RigidBody4D* a, float a_bounce, RigidBody4D* b, float b_bounce, float dt)
{
	...
	for (int i = 0; i < 3; i++)
	{
		glm::vec4 impulse = (1.f + max(a_bounce,b_bounce)) * (c.tangents[i] * (newImpulses[i] - c.contacts.tangentImpulse[i]));
	...
	c.contacts.normalImpulse = max(prevImpulse + lambda0, 0.f);
	glm::vec4 impulse = (1.f + max(a_bounce ,b_bounce)) * c.normal * (c.contacts.normalImpulse - prevImpulse);
	resolve_impulse(a, -impulse, c.contacts.contactPos, dt);
	resolve_impulse(b, impulse, c.contacts.contactPos, dt);
}
...
static void solve(CollisionConstraint c, RigidBody4D* b, float a_bounce, const char* objectNameA, float dt)
{
	...
	for (int i = 0; i < 3; i++)
	{
		glm::vec4 impulse = (1.f + max(a_bounce,b->bounce)) * c.tangents[i] * (newImpulses[i] - c.contacts.tangentImpulse[i]);
		c.contacts.tangentImpulse[i] = newImpulses[i];
		resolve_impulse(b, impulse, c.contacts.contactPos, dt);
	}
	...
	prevImpulse = c.contacts.normalImpulse;
	c.contacts.normalImpulse = max(prevImpulse + lambda0, 0.f);
	glm::vec4 impulse = (1.f + max(a_bounce, b->bounce)) * c.normal * (c.contacts.normalImpulse - prevImpulse);
	resolve_impulse(b, impulse, c.contacts.contactPos, dt);
}
	...
	if (collision->collisionManifold.depth > 0.f)
	{
		if (a->objectName == "Billiard" && b->objectName == "Billiard")
		{
			...
			if (offsetVb>0.f)
			{
				...
				glm::vec4 v0DivideMass = (1.f + max(a->bounce, b->bounce)) * offsetV * normalAtoB / (a->mass + b->mass);
