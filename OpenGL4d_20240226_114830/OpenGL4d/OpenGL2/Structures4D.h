...
static void generate_room4D(glm::vec4 position, glm::vec4 scale, const unsigned int a, const unsigned int b, const unsigned int c, const unsigned int d, const unsigned int e, const unsigned int f, bool isGroundReflect,Texture3D* groundTexture, glm::vec4 groundColor, Shader* groundShader, bool isBaseBoardReflect, Texture3D* baseBoardTexture, glm::vec4 baseBoardColor, Shader* baseBoardShader, bool isWallReflect, Texture3D* wallTexture, glm::vec4 wallColor, Shader* wallShader, bool isTopReflect, Texture3D* topTexture, glm::vec4 topColor, Shader* topShader, std::vector<Wall4D*>* walls4D) {
	if (a != 4 && b != 4&&c!=4&&d!=4&&e!=4&&f!=4) {
		...
		walls4D->push_back(new Wall4D("Hypercube", Hypercube(glm::vec4(scale.x, 0.1f, scale.z, 0.01f) / 2.f), glm::vec4(0, 0.05f, 0, scale.w / 2 - 0.005f) + position, alg::bivec4(), glm::vec4(scale.x, 0.1f, scale.z, 0.01f), 1.6f, 0.25f, isBaseBoardReflect, baseBoardTexture,0.f, baseBoardColor, NONE, baseBoardShader));
	}
	...
	if (a != 3 && b != 3 && c != 3 && d != 3 && e !=3 && f != 3) {
		...
		walls4D->push_back(new Wall4D("Hypercube", Hypercube(glm::vec4(scale.x, 0.1f, 0.01f, scale.w-0.02f) / 2.f), glm::vec4(0, 0.05f, scale.z / 2 - 0.005f, 0) + position, alg::bivec4(), glm::vec4(scale.x, 0.1f, 0.01f, scale.w-0.02f), 1.6f, 0.25f, isBaseBoardReflect, baseBoardTexture,0.f, baseBoardColor, NONE, baseBoardShader));
	}
	...
	if (a != 1 && b != 1 && c != 1 && d != 1 && e != 1 && f != 1) {
		...
		walls4D->push_back(new Wall4D("Hypercube", Hypercube(glm::vec4(0.01f, 0.1f, scale.z - 0.02f, scale.w - 0.02f) / 2.f), glm::vec4(scale.x / 2 - 0.005f, 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.01f, 0.1f, scale.z - 0.02f, scale.w - 0.02f), 1.6f, 0.25f, isBaseBoardReflect, baseBoardTexture,0.f, baseBoardColor, NONE, baseBoardShader));
	}
...
static void generate_window4D(glm::vec4 position, glm::vec4 scale, glm::vec4 windowPos, glm::vec2 windowScale, const unsigned int a, const unsigned int b,bool isFloor, const float floorThickness, bool isBaseBoardReflect,Texture3D* baseBoardTexture, glm::vec4 baseBoardColor, Shader* baseBoardShader, bool isFrame, bool isFrameReflect,Texture3D* frameTexture, glm::vec4 frameColor, Shader* frameShader, bool isWindowReflect, Texture3D* windowTexture, glm::vec4 windowColor, Shader* windowShader, bool isWallReflect, Texture3D* wallTexture, glm::vec4 wallColor, Shader* wallShader, std::vector<Wall4D*> *walls4D) {
	...
	wallPos1[a2] = offset, wallPos1[1] = scale[1] / 2.f, wallPos1[c] = 0, wallPos1[d] = scale[d] / 4 + windowPos[d] / 2 +windowSize2 / 4.f;
	wallPos2[a2] = offset, wallPos2[1] = scale[1] / 2.f, wallPos2[c] = 0, wallPos2[d] = -scale[d] / 4 + windowPos[d] / 2 -windowSize2 / 4.f;
	...
	wallPos4[a2] = offset, wallPos4[1] = (windowPos[1]- windowHeight2/2.f+ floorThickness) / 2.f, wallPos4[c] = windowPos[c], wallPos4[d] = windowPos[d];
	wallPos5[a2] = offset, wallPos5[1] = scale[1] / 2.f, wallPos5[c] = scale[c] / 4 + windowPos[c] / 2 +windowSize2 / 4.f, wallPos5[d] = windowPos[d];
	wallPos6[a2] = offset, wallPos6[1] = scale[1] / 2.f, wallPos6[c] = -scale[c] / 4 + windowPos[c] / 2 -windowSize2 / 4.f, wallPos6[d] = windowPos[d];
	wallScale1[a2] = 0.2f, wallScale1[1] = scale[1], wallScale1[c] = scale[c], wallScale1[d] = scale[d] / 2 - windowPos[d] -windowSize2 / 2.f;
	wallScale2[a2] = 0.2f, wallScale2[1] = scale[1], wallScale2[c] = scale[c], wallScale2[d] = scale[d] / 2 + windowPos[d] -windowSize2 / 2.f;
	...
	wallScale4[a2] = 0.2f, wallScale4[1] = windowPos[1] - windowHeight2 / 2.f+ floorThickness, wallScale4[c] =windowSize2, wallScale4[d] =windowSize2;
	wallScale5[a2] = 0.2f, wallScale5[1] = scale[1], wallScale5[c] = scale[c] / 2 - windowPos[c] -windowSize2 / 2.f, wallScale5[d] =windowSize2;
	wallScale6[a2] = 0.2f, wallScale6[1] = scale[1], wallScale6[c] = scale[c] / 2 + windowPos[c] -windowSize2 / 2.f, wallScale6[d] =windowSize2;
...
static void generate_pool_table4D(glm::vec4 position, glm::vec4 scale, float height, bool isFrameReflect, Texture3D* framecolor, bool isTopReflect, Texture3D* topColor, bool isLagReflect, Texture3D* lagcolor, std::vector<Wall4D*> *walls4D, Shader* shader)
{
	...
	walls4D->push_back(new Wall4D("Hypercube", Hypercube(glm::vec4(scale.x + 0.04f, 0.04f, scale.z + 0.04f, scale.w + 0.04f) / 2.f), glm::vec4(0, height, 0, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.04f, 0.04f, scale.z + 0.04f, scale.w + 0.04f), 0.f, 0.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Hypercube", Noneshape(), glm::vec4(0, height/2.f+0.01f, 0, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.4f, 0.02f+ height, scale.z + 0.4f, scale.w + 0.4f), 0.f, 0.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));

	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(0.025f, 0.1f, scale.z, scale.w) / 2.f), glm::vec4(scale.x / 2.f + 0.0125f, height + 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.025f, 0.1f, scale.z, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(0.025f, 0.1f, scale.z, scale.w) / 2.f), glm::vec4(-scale.x / 2.f - 0.0125f, height + 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.025f, 0.1f, scale.z, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(scale.x + 0.05f, 0.1f, 0.025f, scale.w) / 2.f), glm::vec4(0, height + 0.05f, scale.z / 2.f + 0.0125f, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.05f, 0.1f, 0.025f, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(scale.x + 0.05f, 0.1f, 0.025f, scale.w) / 2.f), glm::vec4(0, height + 0.05f, -scale.z / 2.f - 0.0125f, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.05f, 0.1f, 0.025f, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(scale.x + 0.05f, 0.1f, scale.z + 0.05f, 0.025f) / 2.f), glm::vec4(0, height + 0.05f, 0, scale.w / 2.f + 0.0125f) + position, alg::bivec4(), glm::vec4(scale.x + 0.05f, 0.1f, scale.z + 0.05f, 0.025f), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(scale.x + 0.05f, 0.1f, scale.z + 0.05f, 0.025f) / 2.f), glm::vec4(0, height + 0.05f, 0, -scale.w / 2.f - 0.0125f) + position, alg::bivec4(), glm::vec4(scale.x + 0.05f, 0.1f, scale.z + 0.05f, 0.025f), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f,1.25f,0.5f,1.f), NONE, shader));
	
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(0.04f, 0.1f, (scale.z+0.05f), (scale.w+0.05f)) / 2.f), glm::vec4((scale.x+0.05f)  / 2.f + 0.02f, height+0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.04f, 0.1f, (scale.z+0.05f), (scale.w+0.05f)), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4(0.04f, 0.1f, (scale.z+0.05f), (scale.w+0.05f)) / 2.f), glm::vec4(-(scale.x+0.05f) / 2.f - 0.02f, height + 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.04f, 0.1f, (scale.z+0.05f), (scale.w+0.05f)), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, 0.04f, (scale.w+0.05f)) / 2.f), glm::vec4(0, height + 0.05f, (scale.z+0.05f) / 2.f + 0.02f, 0) + position, alg::bivec4(), glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, 0.04f, (scale.w+0.05f)), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, 0.04f, (scale.w+0.05f)) / 2.f), glm::vec4(0, height + 0.05f, -(scale.z+0.05f) / 2.f - 0.02f, 0) + position, alg::bivec4(), glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, 0.04f, (scale.w+0.05f)), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, (scale.z+0.05f) + 0.08f, 0.04f) / 2.f), glm::vec4(0, height + 0.05f, 0, (scale.w+0.05f) / 2.f + 0.02f) + position, alg::bivec4(), glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, (scale.z+0.05f) + 0.08f, 0.04f), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));
	walls4D->push_back(new Wall4D("Pool", Hypercube(glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, (scale.z+0.05f) + 0.08f, 0.04f) / 2.f), glm::vec4(0, height + 0.05f, 0, -(scale.w+0.05f) / 2.f - 0.02f) + position, alg::bivec4(), glm::vec4((scale.x+0.05f) + 0.08f, 0.1f, (scale.z+0.05f) + 0.08f, 0.04f), 0.f, 1.f, isFrameReflect, framecolor,0.f, glm::vec4(1.f,0.75f,0.5f,1.f), NONE, shader));

	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(scale.x / 2.f + 0.1f, height + 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.2f, 0.6f, scale.z, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(-scale.x / 2.f - 0.1f, height + 0.05f, 0, 0) + position, alg::bivec4(), glm::vec4(0.2f, 0.6f, scale.z, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(0, height + 0.05f, scale.z / 2.f + 0.1f, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.4f, 0.6f, 0.2f, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(0, height + 0.05f, -scale.z / 2.f - 0.1f, 0) + position, alg::bivec4(), glm::vec4(scale.x + 0.4f, 0.6f, 0.2f, scale.w), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(0, height + 0.05f, 0, scale.w / 2.f + 0.1f) + position, alg::bivec4(), glm::vec4(scale.x + 0.4f, 0.6f, scale.z + 0.4f, 0.2f), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
	walls4D->push_back(new Wall4D("Pool", Noneshape(), glm::vec4(0, height + 0.05f, 0, -scale.w / 2.f - 0.1f) + position, alg::bivec4(), glm::vec4(scale.x + 0.4f, 0.6f, scale.z + 0.4f, 0.2f), 0.f, 1.f, isTopReflect, topColor,0.f, glm::vec4(0.5f, 1.25f, 0.5f,1.f), BOX4D, shader));
...
static void generate_billiard4D(glm::vec4 position,Texture3D* color, std::vector<RigidBody4D*>* rigidBodies4D, bool isGravity, Shader* shader)
{
	rigidBodies4D->push_back(new RigidBody4D("Pool", Hypersphere(glm::vec4(0.0525f)), position+glm::vec4(0.f,0.05f,0.f,0.f), alg::bivec4(30, 30, 30, 30, 30, 30), glm::vec4(0.0525f), 0.25f, 0.000675f, 0.f, 1.f, isGravity,false, glm::vec4(0, 0, 0, 0), alg::bivec4(), true, color,0.f, glm::vec4(1.f), SPHERE4D, true, shader));
}
	...
	for (int w = -scale.w * size/2 + size/2;w <= scale.w * size/2 - size/2;w += size) {
		for (int z = -scale.z * size/2 + size/2;z <= scale.z * size/2 - size/2;z += size) {
			for (int x = -scale.x * size/2 + size/2;x <= scale.x * size/2 - size/2;x += size) {
				terrains4D->push_back(new Terrain4D("Terrain", Terrain4d(glm::ivec4(x, 54, z, w), size), glm::vec4(x, 0, z, w), glm::vec4(4.f, 4.f, 4.f, 4.f), 1.6f, 0.25f, TERRAIN4D, shader));
			}
		}
	}
	{
...
static void generate_dynamites4D(glm::vec4 position, Texture3D* color,std::vector<Wall4D*>* dynamites4D, Shader* shader)
{
	dynamites4D->push_back(new Wall4D("Dynamite", Hypercylinder(glm::vec4(0.2f, 0.2f, 1.f, 0.2f) / 2.f), position+glm::vec4(0.f, 0.5f, 0.f, 0.f), alg::bivec4(0.f, 0.f, 0.f, 90.f), glm::vec4(0.2f, 0.2f, 1.f, 0.2f), 1.6f, 0.25f, true, color, 0.f,glm::vec4(0.77f, 0.3f, 0.2f,0.5f) * 2.f, BOX4D, shader));
}

static void generate_dynamites4D(glm::vec4 position, Texture3D* color, std::vector<RigidBody4D*>* dynamites4D, bool isGravity, Shader* shader)
{
	dynamites4D->push_back(new RigidBody4D("Dynamite", Hypercylinder(glm::vec4(0.2f, 0.2f, 1.f, 0.2f) / 2.f), position + glm::vec4(0.f, 0.5f, 0.f, 0.f), alg::bivec4(0.f, 0.f, 0.f, 90.f), glm::vec4(0.2f, 0.2f, 1.f, 0.2f), 1.f, 0.1f, 1.6f, 0.25f, isGravity, false, glm::vec4(0, 0, 0, 0), alg::bivec4(), true, color,0.f, glm::vec4(0.77f, 0.3f, 0.2f,0.5f) * 2.f, BOX4D,true, shader));
}
