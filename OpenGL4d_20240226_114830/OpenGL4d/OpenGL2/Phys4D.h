...
namespace phy {
				...
				for (size_t j = i + 1; j < size_of_objects4d(rigidBodies4D, dynamites4D2); j++)
				{
					if (find_rigidbody4d(rigidBodies4D, dynamites4D2, j)->objectName == "Celestial4D")
					{
						...
						//if (dis >= 0.5f)
						{
							...
							force4D = 10.f * dir_a_to_b_e / pow(dis, 3.f);
		...
		const float dt)
	{
		...
		for (HingeConstraint* i : hingeConstraints)
		{
			if (i->index2 == -1)
			{
				if (i->index1 < size_of_objects4d(rigidBodies4D, dynamites4D2))
		...
		const float dt)
	{
		for (int i = 0; i < 1; i++)
		{
			start_rigidBodies4D(rigidBodies4D, dynamites4D2, dt/1);
			caculate_rigidBodies4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, waters4D, collisions, dt/1);
			update_joints4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, sticks4D, springs4D, hingeConstraints, dt/1);
			update_rigidBodies_position4D(rigidBodies4D, dynamites4D2, dt/1);
	...
	{
		for (Stick4D* i : sticks4D) { update_stick4D(rigidBodies4D, dynamites4D2, groups, walls4D,dynamites4D, i); }
		for (Spring4D* i : springs4D) { update_spring4D(rigidBodies4D, dynamites4D2, groups, walls4D,dynamites4D, i); }
