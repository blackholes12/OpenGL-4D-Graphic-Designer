...
#include"Vertex.h"
#include"triPrism.h"
#include"hyperslope.h"
#include"hypercone.h"
#include"hypercube.h"
#include"quad4d.h"
#include"doublequads4d.h"
#include"box4d.h"
#include"waterbox4d.h"
#include"pentachoron.h"
#include"pyramid4d.h"
#include"hexadecachoron.h"
#include"hexacosichoron.h"
#include"Geometry.h"
#include"hypercube_frame.h"
#include"hypersphere_frame.h"
#include"hypercapsule_frame.h"
#include"particle4d_frame.h"
...
public:
	...
	void set2(glm::vec4* points4D,const unsigned int sizeOfpoints4D, unsigned int* indices4D0, const unsigned int sizeOfTetra, glm::vec4 scale4D)
	{
		...
		else if (this->primitiveName == "Hypersphere")
		{
		...
		for (int i = 0; i < length; i++)
		{
			for (int j = 0; j < length; j++)
			{
				for (int k = 0; k < width; k++)
				{
					...
					this->points4D[k + width * j + width * length * i] = 0.5f*glm::vec4(xyzRw * xzRy * xz.x, xyzRw * xzRy * xz.y, height, w);
					this->normals4D[k + width * j + width * length * i] = normalize(glm::vec4(xyzRw * xzRy * xz.x, xyzRw * xzRy * xz.y, -xyzRw * y, w));
				}
			}
		}
...
public:
	Terrain4d(glm::ivec4 pos, const int size): Primitive4D()
	{
		...
		this->normals4D = new glm::vec4[4 * 6 * size * size * size];
			...
			for (size_t z = 0;z < size;z++) {
				for (size_t x = 0;x < size;x++) {					
					...
					for (size_t i = 0;i < 24;i++) {
						...
						this->normals4D[cubeIndex + i] = this->terrainNormal2[x0 + (size + 1) * z0 + (size + 1) * (size + 1) * w0];
					}
				}
			}
		}
		...
		this->transform_terrain4D(this->points4D, this->normals4D, this->cubeIndex_XZW, this->indices4D0, pos, sizeOfTetra, new Vertex4D[4 * sizeOfTetra], new GLuint[6 * sizeOfTetra]);
	}
	~Terrain4d() {
