...
static void update_point_light4d(std::vector<PointLight4D*> pointLights4D, Shader* type1, Shader* type2)
{
	...
	type1->use();
	type1->set1i(pointLights4D.size(), "D4_PointLightAmount");
	type2->use();
	type2->set1i(pointLights4D.size(), "D4_PointLightAmount");
}
...
public:
	...
	DirectionLight4D(Texture3D* diffuse3D, alg::bivec4 RotateAngle4D, glm::vec3 color, float intensity)
	{
		...
		this->color = color;
		this->intensity = intensity;
		this->mesh4D = new Mesh4D(Particle4d(), glm::vec4(0,0,1,0),
			update(alg::rotor4(), alg::bivec4(0, 0, 0, glm::radians(90.f), 0, 0))
			, glm::vec4(1.f));
		//this->set_light4D_property(type);
	}
	...
	void update_light4D_in_earth_environment(const float period,const float worldTime)
	{
		...
		{
			float distance = 1.4f*65.68501f*(sqrt(this->direction4D.y* this->direction4D.y+0.00269684f) + this->direction4D.y);
			this->color = 1.45f / 1.38f *glm::vec3(exp(-0.1889f * distance), exp(-0.51f * distance), exp(-1.2576f * distance))/1.09f;//1.45f/1.28f
		}
		if (-this->direction4D.y <= -0.15f)
		{
	...
	void set_light4D_property(Shader* type)
	{
		...
		type->set1f(this->intensity, "directionLight4D.intensity");
	...
	void render(Shader* shader)
	{
		this->diffuse3D->bind(0);
		shader->use();
		shader->set1f(this->intensity*0.013f, "intensity");
		shader->setVec4f(glm::vec4(this->color, 1.f), "lightcolor");//Update uniforms	
	...
	{
		shader->use();//Update uniforms	
		shader->set1f(20000.f * 20000.f * 20000.f * this->intensity, "intensity");
		shader->setVec4f(glm::vec4(0.f), "cameraPos");//Update uniforms	
