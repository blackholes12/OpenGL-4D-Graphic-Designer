...
void Game::imgui_use() {
	...
	if (ImGui::TreeNodeEx("Levels", 2)) {
		if (ImGui::BeginCombo("Level", ("Level " + to_string(this->level)).c_str()))
		{
			...
			for (int levelCount = 1; levelCount <= 6; levelCount++)
			{
				...
				{
					this->startLevel(this->level = levelCount,0,"");
				}
			}
			ImGui::EndCombo();
		}
	...
	if (ImGui::TreeNodeEx("Controls", 2))
	{
		...
		{
			this->rigidBodies4D[0] = new RigidBody4D("Player", Hypercube(glm::vec4(0.4f, 1.4f, 0.4f, 0.4f)), glm::vec4(0.f, 64.7f, 0.f, 0.f), alg::bivec4(0.f, 0.f, 0.f, 90.f), glm::vec4(0.4f, 0.4f, 1.4f, 0.4f), 65.f, 0.f, 1.6f, 0.25f, true, false, glm::vec4(0, 0, 0, 0), alg::bivec4(), true, this->wallTexs3D[YELLOWMARBLE], 0.f, glm::vec4(1.f), CAPSULE4D, true, this->shaders[WALLD4]);
		}
