...
{
	glm::vec4 n1 = b - a - normalize(c - a) * dot(normalize(c - a), b - a);
	glm::vec4 n2 = c - b - normalize(a - b) * dot(normalize(a - b), c - b);
	glm::vec4 n3 = a - c - normalize(b - c) * dot(normalize(b - c), a - c);
...
{
	wallPos4D = glm::vec4(floor(hittingPos.x*2.f + 0.5f)/2.f, floor(hittingPos.y * 4.f+0.00001f) / 4.f, floor(hittingPos.z * 2.f + 0.5f) / 2.f, floor(hittingPos.w * 2.f + 0.5f) / 2.f);
