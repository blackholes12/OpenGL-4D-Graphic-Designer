...
Game::~Game()
{
	...
	this->shader_2D_Shape = nullptr;
	delete this->rectangle2D;
	this->rectangle2D = nullptr;
	for (Shader* i : this->shaders) { delete i; i = nullptr; }
	for (Texture* i: this->textures) { delete i; i = nullptr; }
	for (Texture* i : this->pointerTexs) { delete i; i = nullptr; }
	delete this->deleteMap3D;
	this->deleteMap3D = nullptr;
	for (Texture3D* i : this->textures3D) { delete i; i = nullptr; }
	for (Texture3D* i : this->particleTexs3D) { delete i; i = nullptr; }
	for (Texture3D* i : this->wallTexs3D) { delete i; i = nullptr; }
	for (Texture3D* i : this->groundTexs3D) { delete i; i = nullptr; }
	for (Sound* i : this->sounds) { delete i; i = nullptr; }
	for (Music* i : this->musics) { delete i; i = nullptr; }
	for (Object* i : this->objects) { delete i; i = nullptr; }
	delete this->skybox4D;
	this->skybox4D = nullptr;
	delete this->coordinate4D;
	this->coordinate4D = nullptr;
	delete this->targetIcon;
	this->targetIcon = nullptr;
	delete this->demoObjects4D;
	this->demoObjects4D = nullptr;
	for (Demo4D* i : this->demos4D) { delete i; i = nullptr; }
	for (JointLine4D* i : this->dragLines4D) { delete i; i = nullptr; }
	for (Input* i:this->inputs) { delete i; i = nullptr; }
	for (Wall4D* i : this->walls4D) { delete i; i = nullptr; }
	for (Wall4D* i : this->dynamites4D) { delete i; i = nullptr; }
	for (RigidBody4D* i : this->dynamites4D2) { delete i; i = nullptr; }
	for (RigidBody4D* i : this->rigidBodies4D) { delete i; i = nullptr; }
	for (Terrain4D* i : this->terrains4D) { delete i; i = nullptr; }
	for (Water4D* i : this->waters4D) { delete i; i = nullptr; }
	for (Group* i : this->groups) { delete i; i = nullptr; }
	for (Emitter4D* i : this->emitters4D) { delete i; i = nullptr; }
	for (PointLight* i:this->pointLights) { delete i; i = nullptr; }
	for (DirectionLight* i:this->directionLights) { delete i; i = nullptr; }
	for (PointLight4D* i :this->pointLights4D) { delete i; i = nullptr; }
	delete this->directionLight4D;
	this->directionLight4D = nullptr;
	for (Collision* i : this->collisions) { delete i; i = nullptr; }
	for (HingeConstraint4D* i : this->hingeConstraints) { delete i; i = nullptr; }
	for (Stick4D* i : this->sticks4D) { delete i; i = nullptr; }
	for (Spring4D* i : this->springs4D) { delete i; i = nullptr; }
	for (Shift4D* i : this->shifts4D) { delete i; i = nullptr; }
	for (Float4D* i : this->floats4D) { delete i; i = nullptr; }
	for (Rotate4D* i : this->rotates4D) { delete i; i = nullptr; }
}
