...
{
    glm::vec4 a = support_of_cso(s1, s2, s2->position4D - s1->position4D+glm::vec4(1.f));
    *simplex = Simplex(a);
    glm::vec4 d = -a;
...
struct ExpandingPolytope2
{
    ...
    void expand(glm::vec4 newPoint4D)
    {
        ...
        unsigned addVertexIndex = this->vertices4D.size() - 1;
        for (int i = 0; i < this->ePTetras.size(); i++)
        {
        ...
        for (int i = 0; i < addTetrasIndexs.size(); i++)
        {
            normal4D = normalize(cross4d( this->vertices4D[addTetrasIndexs[i][3]] - this->vertices4D[addTetrasIndexs[i][0]], this->vertices4D[addTetrasIndexs[i][2]] - this->vertices4D[addTetrasIndexs[i][0]], this->vertices4D[addTetrasIndexs[i][1]] - this->vertices4D[addTetrasIndexs[i][0]]));
            distance = dot(normal4D, this->vertices4D[addTetrasIndexs[i][0]]);
            if (distance < 0.f)
            {
                this->ePTetras.push_back(EPTetra(addTetrasIndexs[i], -normal4D, -distance));
            }
            else
            {
                this->ePTetras.push_back(EPTetra(addTetrasIndexs[i], normal4D, distance));
...
static glm::vec4 epa2(Object4D* s1, Object4D* s2, unsigned steps, Simplex simplex, float* depth, std::vector<glm::vec4>* supportPoints1, std::vector<glm::vec4>* supportPoints2)
{
    ...
    while (true)
    {
        ...
        if (dot(expandPoint, normal) - distance < 0.00002f)
        {
            break;
        }
    ...
    if (depth0 > 0.f)
    {
        *depth = distance<0.3f?distance:-1.f;
