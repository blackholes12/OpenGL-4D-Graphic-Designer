...
namespace phy {
	...
	static void update_rigidBodies_position4D(std::vector<RigidBody4D*> rigidBodies4D, std::vector<RigidBody4D*> dynamites4D2, std::vector<Group*> groups, vector<Wall4D*> walls4D, vector<Wall4D*> dynamites4D, std::vector<Collision*>* collisions, const float dt)
	{
		...
		//	if (collision->index2 == -1)
		//	{
		//		glm::vec4 shift4D = -0.99f * support(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1), glm::vec4(0, -1, 0, 0)).y*glm::vec4(0, 1, 0, 0) ;
		//		find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->set_position4D(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->position4D + shift4D);
		//	}
		//	else
		//	{
		//		if (collision->index2 < size_of_objects4d(rigidBodies4D, dynamites4D2))
		//		{
		//			float depth = 0.99f*dot(support(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index2), collision->collisionManifold.normal) - support(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1), -collision->collisionManifold.normal), collision->collisionManifold.normal);
		//			glm::vec4 shift4D = depth * collision->collisionManifold.normal;
		//			float massA = rigidBodies4D[collision->index1]->mass;
		//			float massB = rigidBodies4D[collision->index2]->mass;
		//			find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->set_position4D(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->position4D + shift4D * massA / (massA + massB));
		//			find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index2)->set_position4D(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index2)->position4D - shift4D * massB / (massA + massB));
		//		}
		//		if (collision->index2 >= size_of_objects4d(rigidBodies4D, dynamites4D2))
		//		{
		//			float depth = 0.99f * dot(support(find_wall4d(groups, walls4D, dynamites4D, collision->index2- size_of_objects4d(rigidBodies4D, dynamites4D2)), collision->collisionManifold.normal) - support(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1), -collision->collisionManifold.normal), collision->collisionManifold.normal);
		//			glm::vec4 shift4D = depth * collision->collisionManifold.normal;
		//			find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->set_position4D(find_rigidbody4d(rigidBodies4D, dynamites4D2, collision->index1)->position4D + shift4D);
		//		}
		//	}
		...
		const float dt)
	{
		...
		for (int i = steps-1; i >= 0; i--)
		{
			...
			caculate_rigidBodies4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, waters4D, collisions, dt2);
			update_joints4D(rigidBodies4D, dynamites4D2, groups, walls4D, dynamites4D, sticks4D, springs4D, hingeConstraints, dt2);
